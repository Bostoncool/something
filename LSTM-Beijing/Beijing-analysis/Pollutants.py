...
def process_pollutant_data(df):
    """
    处理数据，统一格式，以便筛选污染物超标的数据
    """
    try:
        print("开始处理数据...")
        ...
        # 筛选包含污染物的行（考虑到可能有数字前缀）
        pollutant_df = df[df['type'].astype(str).str.contains('O3_24h', regex=True, case=False)].copy()
        print(f"O3_24h数据筛选后记录数: {len(pollutant_df)}")
        
        if len(pollutant_df) == 0:
            print("警告：未找到O3_24h数据")
            return pd.DataFrame()
        
        # 获取日期信息（如果需要）
        if 'date' not in pollutant_df.columns:
            # 从type列或文件名中提取日期
            date_pattern = r'(\d{8})'
            pollutant_df['date'] = pollutant_df['type'].astype(str).str.extract(date_pattern)[0]
        
        # 将宽格式转换为长格式
        # 排除非数据列
        non_data_cols = ['type', 'date']
        station_cols = [col for col in pollutant_df.columns if col not in non_data_cols]
        
        # 转换为长格式
        melted_df = pd.melt(pollutant_df,
                           id_vars=['date', 'type'],
...
def analyze_heavy_pollution(df):
    ...
    # 筛选污染物超标的数据
    heavy_pollution = df[df['value'] > 265].copy()
    ...
    print(f"O3_24h超标天数(O3_24h>265): {heavy_days}天")
    print(f"O3_24h超标发生率: {heavy_rate:.2f}%")
    ...
    plt.title('各站点O3_24h超标天气分布')
    
    plt.tight_layout()
    
    # 保存图表到桌面
    desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
    plt.savefig(os.path.join(desktop_path, 'O3_24h超标天气分析.png'), dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    # 处理数据
    processed_data = process_pollutant_data(raw_data)
    ...
    save_data_to_desktop(heavy_pollution, f"O3_24h超标天气数据_{timestamp}.csv")
    
    # 按年份、月份、季节汇总统计O3_24h超标天气
    ...
    # 保存汇总数据
    save_data_to_desktop(yearly_summary, f"O3_24h超标天气年度统计_{timestamp}.csv")
    save_data_to_desktop(monthly_summary, f"O3_24h超标天气月度统计_{timestamp}.csv")
    save_data_to_desktop(seasonal_summary, f"O3_24h超标天气季节统计_{timestamp}.csv")
