...
# 绘制图表
p <- ggplot(data, aes(x = Concentration_Max, y = MAE, color = Model, group = Model)) +
  ...
  labs(
    title = "The trend of MAE values for different models",
...
# 方法2：矢量图（绝对清晰）
ggsave("plot_vector_MAE.svg", plot = p, width = 10, height = 7)
