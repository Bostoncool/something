...
for file_path in file_paths:
    try:
        ...
        # 筛选日期范围：2024年01月01日至2024年12月31日
        start_date = pd.to_datetime('2024-01-01')
        end_date = pd.to_datetime('2024-12-31')
        date_filtered_data = data[(data[date_col] >= start_date) & (data[date_col] <= end_date) &
        ...
        # 对每个浓度范围进行计算
        for min_val, max_val, range_label in concentration_ranges:
            # 筛选符合当前浓度范围的数据
            filtered_data = date_filtered_data[
                (date_filtered_data[actual_col] >= min_val) & 
                (date_filtered_data[actual_col] <= max_val)
            ]

            # 检查过滤后的数据是否为空
            if len(filtered_data) == 0:
                print(f"警告: {model_name} 在浓度范围 {range_label} 过滤后没有数据，跳过此范围")
                continue
            ...
            results.append({
                'Model': model_name,
                'Concentration_Range': range_label,
                'R2': r2,
                'RMSE': rmse,
                'MAE': mae,
                'MAPE': mape,
                'Sample_Count': len(filtered_data)  # 添加样本数量信息
            })
...
# 按模型和浓度范围排序，便于查看
results_df = results_df.sort_values(['Model', 'Concentration_Range'])

# 保存结果为CSV文件
results_df.to_csv('九龙拉棺.csv', index=False)

print(f"模型评估结果已保存为 '九龙拉棺.csv'")
print(f"共处理 {len(results_df)} 条记录，涵盖 {len(concentration_ranges)} 个浓度范围")
print(f"浓度范围: {', '.join([r[2] for r in concentration_ranges])}")
