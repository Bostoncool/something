"""
北京PM2.5浓度预测 - CNN-LSTM + Attention模型 (GPU加速版)
使用1D CNN + LSTM + Attention进行时间序列预测

GPU加速特性:
- 自动混合精度训练 (AMP) - 2-3倍速度提升
- 优化的数据加载 (pin_memory, 多进程)
- 增大批处理大小 (128)
- GPU内存监控和管理
- 梯度裁剪防止爆炸
- 训练速度统计
...
if torch.cuda.is_available():
    ...
    torch.cuda.manual_seed_all(42)  # 多GPU支持
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = True  # GPU加速：自动寻找最优卷积算法

# 设置设备
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

print("=" * 80)
print("北京PM2.5浓度预测 - CNN-LSTM + Attention模型 (GPU加速版)")
print("=" * 80)
print(f"使用设备: {device}")
if torch.cuda.is_available():
    ...
    print("⚠️  警告: 未检测到GPU，将使用CPU模式（速度较慢）")
    print("   混合精度训练: 已禁用")
...
# GPU优化参数
BATCH_SIZE = 128 if torch.cuda.is_available() else 64  # GPU使用更大批次
NUM_WORKERS = 4  # 数据加载进程数
PIN_MEMORY = torch.cuda.is_available()  # 锁页内存加速传输
PERSISTENT_WORKERS = True if torch.cuda.is_available() else False  # 保持工作进程
...
print(f"  锁页内存 (Pin Memory): {PIN_MEMORY}")
print(f"  持久化工作进程: {PERSISTENT_WORKERS}")
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, patience, device):
    ...
    if epoch_times:
        ...
        print(f"    平均每epoch: {avg_epoch_time:.1f} 秒")
        print(f"    平均速度: {len(train_loader.dataset)/avg_epoch_time:.0f} samples/s")
...
# ============================== 第9部分: 多窗口模型训练 (GPU加速) ==============================
print("\n" + "=" * 80)
print("第3步: 多窗口CNN-LSTM模型训练 (GPU加速版)")
print("=" * 80)
...
for seq_length in sequence_lengths:
    ...
    if BAYESIAN_OPT_AVAILABLE:
        ...
        def cnn_lstm_evaluate(hidden_size, num_layers, num_filters, kernel_size, learning_rate, dropout):
            ...
            model_temp, _, _ = train_model(
                ...
                num_epochs=30, patience=10, device=device  # 减少epoch数，GPU训练更快
            )
                    ...
                    for ks in param_grid['kernel_size']:
                        for lr in param_grid['learning_rate']:
                            for dr in param_grid['dropout']:
                                ...
                                model_temp, _, _ = train_model(
                                    model_temp, train_loader, val_loader, criterion_temp, optimizer_temp,
                                    num_epochs=30, patience=10, device=device  # 减少epoch数
                                )
    ...
    print(f"保存模型: cnn_lstm_window{seq_length}_optimized_gpu.pth/pkl")
...
for seq_length in sequence_lengths:
    ...
    axes[0].set_title(f'CNN-LSTM基础模型训练过程 (窗口={seq_length}天) [GPU]', fontsize=13, fontweight='bold')
    ...
    axes[1].set_title(f'CNN-LSTM优化模型训练过程 (窗口={seq_length}天) [GPU]', fontsize=13, fontweight='bold')
    axes[1].legend(fontsize=10)
    axes[1].grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(output_dir / f'training_curves_window{seq_length}_gpu.png', dpi=300, bbox_inches='tight')
    print(f"保存: training_curves_window{seq_length}_gpu.png")
    plt.close()
...
for i, seq_length in enumerate(sequence_lengths):
    for j, model_type in enumerate(['basic', 'optimized']):
        ...
        ax.set_title(f'CNN-LSTM-{"基础" if model_type=="basic" else "优化"} (窗口={seq_length}天) [GPU]\nR²={r2:.4f}, RMSE={rmse:.2f}', 
...
print("保存: prediction_scatter_gpu.png")
plt.close()
...
axes[0].set_title(f'CNN-LSTM基础模型 - 时间序列预测对比（窗口={best_window}天，最后{plot_range}个样本） [GPU]', 
...
axes[1].set_title(f'CNN-LSTM优化模型 - 时间序列预测对比（窗口={best_window}天，最后{plot_range}个样本） [GPU]', 
                  fontsize=13, fontweight='bold')
axes[1].legend(fontsize=10)
axes[1].grid(True, alpha=0.3)
...
print("保存: timeseries_comparison_gpu.png")
plt.close()
...
print("保存: residuals_analysis_gpu.png")
plt.close()
...
for i, seq_length in enumerate(sequence_lengths):
    ...
    axes[i].set_title(f'Attention权重分布 (窗口={seq_length}天) [GPU]', fontsize=13, fontweight='bold')
    axes[i].grid(True, alpha=0.3, axis='y')

plt.tight_layout()
plt.savefig(output_dir / 'attention_weights_gpu.png', dpi=300, bbox_inches='tight')
print("保存: attention_weights_gpu.png")
plt.close()
...
for i, metric in enumerate(metrics):
    ...
    if metric == 'R²':
        axes[i].set_title(f'{metric} 对比 [GPU]\n(越大越好)', fontsize=12, fontweight='bold')
    else:
...
print("保存: model_comparison_gpu.png")
plt.close()
...
axes[0].set_title(f'基础模型 - 预测误差分布 (窗口={best_window}天) [GPU]\n均值={errors_basic.mean():.2f}, 标准差={errors_basic.std():.2f}', 
                  fontsize=13, fontweight='bold')
...
axes[1].set_title(f'优化模型 - 预测误差分布 (窗口={best_window}天) [GPU]\n均值={errors_opt.mean():.2f}, 标准差={errors_opt.std():.2f}', 
                  fontsize=13, fontweight='bold')
...
print("保存: error_distribution_gpu.png")
plt.close()
...
axes[0].set_title('不同窗口大小的R²对比 [GPU]', fontsize=13, fontweight='bold')
axes[0].set_xticks(x)
...
axes[1].set_title('不同窗口大小的RMSE对比 [GPU]', fontsize=13, fontweight='bold')
axes[1].set_xticks(x)
...
print("保存: window_size_comparison_gpu.png")
plt.close()
...
for seq_length in sequence_lengths:
    ...
    print(f"保存: attention_weights_window{seq_length}_gpu.csv (实际长度: {actual_length})")
...
for seq_length in sequence_lengths:
    for model_type in ['basic', 'optimized']:
        ...
        pred_df.to_csv(output_dir / f'predictions_{model_type}_window{seq_length}_gpu.csv', 
                      index=False, encoding='utf-8-sig')
        print(f"保存: predictions_{model_type}_window{seq_length}_gpu.csv")
...
print("保存: best_parameters_gpu.csv")
...
print("\n生成的文件:")
print("\nCSV文件:")
print("  - model_performance_gpu.csv           模型性能对比")
print("  - window_comparison_gpu.csv           窗口大小对比")
print("  - attention_weights_window*_gpu.csv   各窗口Attention权重")
print("  - predictions_*_gpu.csv               各模型预测结果")
print("  - best_parameters_gpu.csv             最佳参数")

print("\n图表文件:")
print("  - training_curves_window*_gpu.png     训练过程曲线（每个窗口）")
print("  - prediction_scatter_gpu.png          预测vs实际散点图")
print("  - timeseries_comparison_gpu.png       时间序列对比")
print("  - residuals_analysis_gpu.png          残差分析")
print("  - attention_weights_gpu.png           Attention权重分布")
print("  - model_comparison_gpu.png            模型性能对比")
print("  - error_distribution_gpu.png          误差分布")
print("  - window_size_comparison_gpu.png      窗口大小对比")

print("\n模型文件:")
for seq_length in sequence_lengths:
    print(f"  - cnn_lstm_window{seq_length}_optimized_gpu.pth   PyTorch模型权重")
    print(f"  - cnn_lstm_window{seq_length}_optimized_gpu.pkl   完整模型对象")
print("  - scaler_X_gpu.pkl                    特征标准化器")
print("  - scaler_y_gpu.pkl                    目标标准化器")
...
print("\n" + "=" * 80)
print("CNN-LSTM PM2.5浓度预测完成！(GPU加速版)")
print("=" * 80)
