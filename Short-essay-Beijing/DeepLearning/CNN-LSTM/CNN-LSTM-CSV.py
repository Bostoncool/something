...
def slugify(name: str) -> str:
    """Make a stable filename slug similar to RF-CSV naming usage."""
...
def get_optimal_batch_size(model_class, input_size, seq_length, device, 
    ...
    # 显式使用关键字参数，避免不同 torch 版本/别名实现导致的“位置参数错位”，
    # 进而把字符串（例如 'cuda'）误当作 init_scale 传入，触发 torch.full(..., fill_value='cuda') 的 TypeError。
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=400):
        ...
        # 计算 div_term：对于偶数索引位置
        # sin 项数量：ceil(d_model/2)，cos 项数量：floor(d_model/2)
        num_sin = (d_model + 1) // 2  # ceil(d_model/2)
        num_cos = d_model // 2  # floor(d_model/2)
        
        # div_term 的长度应该是 num_sin（因为偶数索引最多有 num_sin 个）
        div_term = torch.exp(
            torch.arange(0, num_sin, dtype=torch.float32) * (-math.log(10000.0) / d_model)
        )
        
        # 填充 sin 项（偶数索引：0, 2, 4, ...），使用全部 div_term
        pe[:, 0::2] = torch.sin(position * div_term)
        # 填充 cos 项（奇数索引：1, 3, 5, ...），使用前 num_cos 个 div_term
        if num_cos > 0:
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, patience, device):
    ...
    scaler = GradScaler(init_scale=65536.0) if use_amp else None
...
# Save scaler
print("\nNote: scaler objects will NOT be saved in this script (CSV-only export mode).")
...
def predict_ordered(model, dataset, device, batch_size):
    """Predict on a Dataset with deterministic order (shuffle=False)."""
    loader = DataLoader(
        dataset,
    ...
    with torch.no_grad():
        for X_batch, y_batch in loader:
            ...
            trues.append(y_batch.detach().cpu().numpy().reshape(-1))
    return np.concatenate(preds), np.concatenate(trues)

def predict_ordered_with_attention(model, dataset, device, batch_size):
    """Predict + attention with deterministic order (shuffle=False)."""
    loader = DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
    ...
    with torch.no_grad():
        for X_batch, y_batch in loader:
            ...
            preds.append(outputs.detach().cpu().numpy().reshape(-1))
            trues.append(y_batch.detach().cpu().numpy().reshape(-1))
            attn_chunks.append(attn_weights.detach().cpu().numpy())
    avg_attention = np.concatenate(attn_chunks, axis=0).mean(axis=0).reshape(-1)
    return np.concatenate(preds), np.concatenate(trues), avg_attention
...
for seq_length in sequence_lengths:
    ...
    # Keep time index for CSV export (align with TimeSeriesDataset offset)
    all_dates = df_combined.index
    dates_train = all_dates[:train_size]
    dates_val = all_dates[train_size:train_size + val_size]
    dates_test = all_dates[train_size + val_size:]
    ...
    all_params[f'cnn_lstm_basic_w{seq_length}'] = {**basic_params}
    ...
    # CSV export: training curves (basic)
    model_slug = f"cnn_lstm_basic_w{seq_length}"
    curves_df = pd.DataFrame({
        'epoch': np.arange(1, len(train_losses_basic) + 1, dtype=int),
        'train_loss': train_losses_basic,
        'val_loss': val_losses_basic,
    })
    save_csv(curves_df, output_dir / f'plot_training_curves__{model_slug}.csv')
    ...
    if BAYESIAN_OPT_AVAILABLE:
        ...
        def cnn_lstm_evaluate(hidden_size, num_layers, num_filters, kernel_size, learning_rate, dropout):
            ...
            # 确保 hidden_size 是 n_heads (4) 的倍数，否则 Multi-Head Attention 会报错
            hidden_size_int = int(hidden_size)
            n_heads = 4
            hidden_size_int = (hidden_size_int // n_heads) * n_heads  # 向下取整到最近的4的倍数
            if hidden_size_int < n_heads:
                hidden_size_int = n_heads  # 至少为4
            ...
            try:
                ...
                # 如果模型创建失败，返回一个很大的负值（很差的分数）
                print(f"Warning: Model creation failed with params {params_test}: {e}")
                return -1e6
            ...
            try:
                ...
                # 如果训练或评估失败，清理内存并返回很差的分数
                print(f"Warning: Training/evaluation failed with params {params_test}: {e}")
                try:
                    del model_temp, criterion_temp, optimizer_temp
                except:
                    pass
        ...
        # 确保 hidden_size 是 n_heads (4) 的倍数
        n_heads = 4
        hidden_size_int = best_params_opt['hidden_size']
        hidden_size_int = (hidden_size_int // n_heads) * n_heads
        if hidden_size_int < n_heads:
            hidden_size_int = n_heads
        best_params_opt['hidden_size'] = hidden_size_int
    ...
    # 确保 hidden_size 是 n_heads (4) 的倍数（双重检查）
    n_heads = 4
    if 'hidden_size' in best_params_opt:
        hidden_size_int = int(best_params_opt['hidden_size'])
        hidden_size_int = (hidden_size_int // n_heads) * n_heads
        if hidden_size_int < n_heads:
            hidden_size_int = n_heads
        best_params_opt['hidden_size'] = hidden_size_int
    ...
    # 确保 n_heads 参数存在
    if 'n_heads' not in optimized_params:
        optimized_params['n_heads'] = n_heads
    ...
    all_params[f'cnn_lstm_optimized_w{seq_length}'] = {**optimized_params, 'learning_rate': best_lr}
    ...
    # CSV export: training curves (optimized)
    model_slug = f"cnn_lstm_optimized_w{seq_length}"
    curves_df = pd.DataFrame({
        'epoch': np.arange(1, len(train_losses_opt) + 1, dtype=int),
        'train_loss': train_losses_opt,
        'val_loss': val_losses_opt,
    })
    save_csv(curves_df, output_dir / f'plot_training_curves__{model_slug}.csv')

    # Extract Attention weights + CSV export (optimized, deterministic order)
    print("\nExtracting Attention weights (CSV export)...")
    _, _, avg_attention_opt = predict_ordered_with_attention(model_optimized, test_dataset, device, BATCH_SIZE)
    all_attention_weights[f'optimized_w{seq_length}'] = avg_attention_opt
    attn_df = pd.DataFrame({
        'sequence_position': np.arange(len(avg_attention_opt), dtype=int),
        'attention_weight': avg_attention_opt
    })
    save_csv(attn_df, output_dir / f'plot_attention_weights__{model_slug}.csv')

    # Also export attention weights for basic model (for parity)
    _, _, avg_attention_basic = predict_ordered_with_attention(model_basic, test_dataset, device, BATCH_SIZE)
    all_attention_weights[f'basic_w{seq_length}'] = avg_attention_basic
    attn_df_basic = pd.DataFrame({
        'sequence_position': np.arange(len(avg_attention_basic), dtype=int),
        'attention_weight': avg_attention_basic
    })
    save_csv(attn_df_basic, output_dir / f'plot_attention_weights__cnn_lstm_basic_w{seq_length}.csv')

    # CSV export: ordered predictions/residuals/scatter/time-series for both models (train/val/test)
    export_specs = [
        ('cnn_lstm_basic_w', model_basic),
        ('cnn_lstm_optimized_w', model_optimized),
    ]
    split_specs = [
        ('train', train_dataset, dates_train),
        ('val', val_dataset, dates_val),
        ('test', test_dataset, dates_test),
    ]
    for prefix, model_obj in export_specs:
        mslug = f"{prefix}{seq_length}"
        # parameters (one-row)
        params = all_params.get(mslug, {})
        if params:
            save_csv(pd.DataFrame([params]), output_dir / f'parameters__{mslug}.csv')
        for split_name, ds_obj, dt_index in split_specs:
            preds_scaled, trues_scaled = predict_ordered(model_obj, ds_obj, device, BATCH_SIZE)
            preds = scaler_y.inverse_transform(preds_scaled.reshape(-1, 1)).reshape(-1)
            trues = scaler_y.inverse_transform(trues_scaled.reshape(-1, 1)).reshape(-1)

            # align time (drop first seq_length positions)
            times = pd.to_datetime(dt_index[seq_length:]).to_series().reset_index(drop=True)
            x_axis = np.arange(len(times), dtype=int)
            ...
            save_csv(df_pred, output_dir / f'predictions__{mslug}__{split_name}.csv')

            # scatter (mainly useful on test split, but export for all splits)
            scatter_df = df_pred[['time', 'y_true', 'y_pred']].rename(columns={
                'y_true': 'Actual_PM25',
                'y_pred': 'Predicted_PM25',
            })
            save_csv(scatter_df, output_dir / f'plot_scatter__{mslug}__{split_name}.csv')

            # residuals
            residuals_df = df_pred[['time', 'y_true', 'y_pred', 'error']].rename(columns={'error': 'residual'})
            save_csv(residuals_df, output_dir / f'plot_residuals__{mslug}__{split_name}.csv')

            # sampled time-series (last year, for val/test; train may be too long but still safe)
            tail_n = min(365, len(df_pred))
            ts_subset = df_pred.iloc[-tail_n:].copy().reset_index(drop=True)
            step = 4
            ts_sampled = ts_subset.iloc[::step].copy().reset_index(drop=True)
            ts_sampled['x_axis'] = np.arange(len(ts_sampled), dtype=int)
            save_csv(
                ts_sampled[['x_axis', 'time', 'y_true', 'y_pred']],
                output_dir / f'plot_ts_lastyear_sampled__{mslug}__{split_name}.csv'
            )
...
# ============================== Part 11: CSV Export for Plotting ==============================
print("\n" + "=" * 80)
print("Step 5: Exporting Plot CSV Files (No Image Generation)")
print("=" * 80)

# 11.1 Metrics tables (CSV only)
save_csv(results_df, output_dir / 'metrics__all_models_train_val_test.csv')

for model_name in results_df['Model'].unique():
    model_slug = slugify(model_name)
    save_csv(results_df[results_df['Model'] == model_name], output_dir / f'metrics__{model_slug}__train_val_test.csv')
    for ds_name in results_df['Dataset'].unique():
        part = results_df[(results_df['Model'] == model_name) & (results_df['Dataset'] == ds_name)]
        if not part.empty:
            save_csv(part, output_dir / f'metrics__{model_slug}__{ds_name.lower()}.csv')

# 11.2 Test ranking (for metrics comparison plots)
save_csv(test_results, output_dir / 'plot_metrics_ranking__test_only.csv')

# 11.3 Window comparison (test-only, basic vs optimized)
window_comparison = []
for seq_length in sequence_lengths:
    ...
    window_comparison.append({
        ...
        'basic_mae': basic_row['MAE'],
        'basic_mape': basic_row['MAPE'],
        'optimized_r2': opt_row['R²'],
        'optimized_rmse': opt_row['RMSE'],
        'optimized_mae': opt_row['MAE'],
        'optimized_mape': opt_row['MAPE'],
    })
window_df = pd.DataFrame(window_comparison).sort_values('window')
save_csv(window_df, output_dir / 'plot_window_comparison__test_only.csv')
...
save_csv(results_df, output_dir / 'model_performance_gpu.csv')

# Save window comparison
save_csv(window_df, output_dir / 'window_comparison_gpu.csv')

# Save Attention weights (separately per model/window; naming aligned with RF style)
for seq_length in sequence_lengths:
    for model_key in [f'basic_w{seq_length}', f'optimized_w{seq_length}']:
        attn_weights = all_attention_weights.get(model_key)
        if attn_weights is None:
            continue
        model_slug = f"cnn_lstm_{'basic' if model_key.startswith('basic') else 'optimized'}_w{seq_length}"
        attention_df = pd.DataFrame({
            'sequence_position': np.arange(len(attn_weights), dtype=int),
            'attention_weight': attn_weights
        })
        save_csv(attention_df, output_dir / f'attention_weights__{model_slug}.csv')

# Save predictions for each window (legacy combined style, keep as CSV for compatibility)
for seq_length in sequence_lengths:
    for model_type in ['basic', 'optimized']:
        ...
        save_csv(pred_df, output_dir / f'predictions_{model_type}_window{seq_length}_gpu.csv')
...
save_csv(best_params_df, output_dir / 'best_parameters_gpu.csv')
...
print("\nGenerated files:")
print("\nCSV files (examples):")
print("  - metrics__all_models_train_val_test.csv")
print("  - metrics__<model>__train/val/test.csv")
print("  - plot_training_curves__cnn_lstm_<basic|optimized>_w*.csv")
print("  - predictions__cnn_lstm_<basic|optimized>_w*__<train|val|test>.csv")
print("  - plot_scatter__cnn_lstm_<basic|optimized>_w*__<train|val|test>.csv")
print("  - plot_residuals__cnn_lstm_<basic|optimized>_w*__<train|val|test>.csv")
print("  - plot_ts_lastyear_sampled__cnn_lstm_<basic|optimized>_w*__<train|val|test>.csv")
print("  - plot_window_comparison__test_only.csv")
print("  - attention_weights__cnn_lstm_<basic|optimized>_w*.csv")
