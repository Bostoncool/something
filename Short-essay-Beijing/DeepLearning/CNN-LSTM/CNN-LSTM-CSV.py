"""
Beijing PM2.5 Concentration Prediction - CNN-LSTM + Attention Model
...
print("=" * 80)
print("Beijing PM2.5 Concentration Prediction - CNN-LSTM + Attention Model")
print("=" * 80)
...
# ============================== Part 9: Multi-Window Model Training ==============================
print("\n" + "=" * 80)
print("Step 3: Multi-Window CNN-LSTM Model Training")
print("=" * 80)
...
for seq_length in sequence_lengths:
    ...
    print(f"Saved model: cnn_lstm_window{seq_length}_optimized.pth/pkl")
...
for seq_length in sequence_lengths:
    ...
    axes[0].set_title(f'CNN-LSTM Basic Model Training Process (window={seq_length} days)', fontsize=13, fontweight='bold')
    ...
    axes[1].set_title(f'CNN-LSTM Optimized Model Training Process (window={seq_length} days)', fontsize=13, fontweight='bold')
    axes[1].legend(fontsize=10)
    axes[1].grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(output_dir / f'training_curves_window{seq_length}.png', dpi=300, bbox_inches='tight')
    print(f"Saved: training_curves_window{seq_length}.png")
    plt.close()
...
for i, seq_length in enumerate(sequence_lengths):
    for j, model_type in enumerate(['basic', 'optimized']):
        ...
        ax.set_title(f'CNN-LSTM-{"Basic" if model_type=="basic" else "Optimized"} (window={seq_length} days)\nR²={r2:.4f}, RMSE={rmse:.2f}', 
...
axes[0].set_title(f'CNN-LSTM Basic Model - Time Series Prediction Comparison (window={best_window} days, last {plot_range} samples)', 
...
axes[1].set_title(f'CNN-LSTM Optimized Model - Time Series Prediction Comparison (window={best_window} days, last {plot_range} samples)', 
                  fontsize=13, fontweight='bold')
axes[1].legend(fontsize=10)
axes[1].grid(True, alpha=0.3)
...
for i, seq_length in enumerate(sequence_lengths):
    ...
    axes[i].set_title(f'Attention Weight Distribution (window={seq_length} days)', fontsize=13, fontweight='bold')
...
axes[0].set_title(f'Basic Model - Prediction Error Distribution (window={best_window} days)\nMean={errors_basic.mean():.2f}, Std={errors_basic.std():.2f}', 
                  fontsize=13, fontweight='bold')
...
axes[1].set_title(f'Optimized Model - Prediction Error Distribution (window={best_window} days)\nMean={errors_opt.mean():.2f}, Std={errors_opt.std():.2f}', 
                  fontsize=13, fontweight='bold')
...
axes[0].set_title('R² Comparison for Different Window Sizes', fontsize=13, fontweight='bold')
axes[0].set_xticks(x)
...
axes[1].set_title('RMSE Comparison for Different Window Sizes', fontsize=13, fontweight='bold')
axes[1].set_xticks(x)
...
for seq_length in sequence_lengths:
    ...
    print(f"Saved: attention_weights_window{seq_length}.csv (actual length: {actual_length})")
...
for seq_length in sequence_lengths:
    for model_type in ['basic', 'optimized']:
        ...
        pred_df.to_csv(output_dir / f'predictions_{model_type}_window{seq_length}.csv', 
                      index=False, encoding='utf-8-sig')
        print(f"Saved: predictions_{model_type}_window{seq_length}.csv")
...
print("  - window_comparison.csv           Window size comparison")
print("  - attention_weights_window*.csv   Attention weights for each window")
print("  - predictions_*.csv               Prediction results for each model")
print("  - best_parameters.csv             Best parameters")

print("\nChart files:")
print("  - training_curves_window*.png     Training process curves (for each window)")
...
print("  - attention_weights.png           Attention weight distribution")
print("  - model_comparison.png            Model performance comparison")
print("  - error_distribution.png          Error distribution")
print("  - window_size_comparison.png      Window size comparison")

print("\nModel files:")
for seq_length in sequence_lengths:
    print(f"  - cnn_lstm_window{seq_length}_optimized.pth   PyTorch model weights")
    print(f"  - cnn_lstm_window{seq_length}_optimized.pkl   Complete model object")
...
print("\n" + "=" * 80)
print("CNN-LSTM PM2.5 Concentration Prediction Complete!")
print("=" * 80)
