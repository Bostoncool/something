...
def permutation_importance_time_series(
    model: nn.Module,
    X_base: torch.Tensor,   # shape: [N, F] (scaled)
    y_base: torch.Tensor,   # shape: [N]   (scaled)
    ...
    """
    时序场景下的 Permutation Importance：
    - 在"原始时间行"级别（N行）置换某一特征列（整列打乱），再重新构造滑窗数据集评估 Test 指标。
    - 输出每个特征对 RMSE/R2 的平均影响（n_repeats 次置换取均值）。
    """
    model.eval()
    g = torch.Generator(device="cpu")
    g.manual_seed(seed)

    # 可选：为了速度，截取最后 max_rows 行（更贴近"最近一年/近期"分布）
    if max_rows is not None and X_base.shape[0] > max_rows:
        X_base = X_base[-max_rows:].contiguous()
        y_base = y_base[-max_rows:].contiguous()

    # baseline
    base_ds = TensorTimeSeriesDataset(X_base, y_base, seq_length)
    base_loader = create_dataloader(base_ds, batch_size=batch_size, shuffle=False)
    base_metrics, _, _ = evaluate_model(model, base_loader, scaler_y, device)
    ...
    for j in range(n_features):
        ...
        for r in range(n_repeats):
            ...
            # 置换第 j 列：在时间行维度 N 上打乱（保持列的边际分布不变）
            perm = torch.randperm(Xp.shape[0], generator=g)
            Xp[:, j] = Xp[perm, j]

            ds = TensorTimeSeriesDataset(Xp, y_base, seq_length)
            loader = create_dataloader(ds, batch_size=batch_size, shuffle=False)
            m, _, _ = evaluate_model(model, loader, scaler_y, device)
        ...
        rows.append(
            {
                ...
                "permuted_RMSE_mean": rmse_mean,
                "delta_RMSE": rmse_mean - base_rmse,  # 越大越重要（性能变差越多）
                "baseline_R2": base_r2,
                "permuted_R2_mean": r2_mean,
                "delta_R2": base_r2 - r2_mean,        # 越大越重要（R2下降越多）
                "n_repeats": n_repeats,
                "max_rows": -1 if max_rows is None else int(max_rows),
            }
        )

    df_imp = pd.DataFrame(rows).sort_values("delta_RMSE", ascending=False).reset_index(drop=True)
    return df_imp
...
def main():
    ...
    # 6) Permutation Importance（Test）
    print("\nComputing Permutation Importance (Test)...")

    X_test_base = tensors["test"]["X"].cpu()
    y_test_base = tensors["test"]["y"].cpu()

    imp_df = permutation_importance_time_series(
        model=model,
        X_base=X_test_base,
        y_base=y_test_base,
        ...
        batch_size=BATCH_SIZE,
        n_repeats=3,        # 可改：5/10 更稳但更慢
        seed=42,
        max_rows=None,      # 可改：例如 60000 提速（用最后 6万行）
    )

    save_csv(
        imp_df,
        output_dir / f"feature_importance__permutation__{model_name}.csv",
        index=False,
    )
