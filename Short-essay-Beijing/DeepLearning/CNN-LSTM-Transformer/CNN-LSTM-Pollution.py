...
def plot_scatter(y_true, y_pred, path: Path):
    ...
    plt.scatter(y_true, y_pred, alpha=0.4, s=18, edgecolors="none")
...
def plot_timeseries(y_true, y_pred, path: Path, window: int = 400):
    plt.figure(figsize=(14, 5))
    start = max(0, len(y_true) - window)
    idx = np.arange(start, len(y_true))
    plt.plot(idx, y_true[start:], label="Actual", linewidth=1.0, color="black")
    plt.plot(idx, y_pred[start:], label="Prediction", linewidth=1.0, color="tab:green", linestyle="--")
...
def plot_error_distribution(y_true, y_pred, path: Path):
    errors = y_true - y_pred
    fig, axes = plt.subplots(1, 2, figsize=(12, 4))

    axes[0].hist(errors, bins=50, color="steelblue", alpha=0.8, edgecolor="black")
    axes[0].axvline(0, color="red", linestyle="--", linewidth=2)
    axes[0].set_title(
        f"Error Distribution | μ={errors.mean():.2f}, σ={errors.std():.2f}"
    )
    axes[0].set_xlabel("Error (μg/m³)")
    axes[0].set_ylabel("Frequency")
    axes[0].grid(alpha=0.3)

    axes[1].scatter(y_pred, errors, alpha=0.4, s=18, edgecolors="none")
...
def main():
    ...
    training_curve_path = output_dir / "training_curve_pollution.png"
    plot_training_curves(history, training_curve_path)

    test_true = preds["Test"]["y_true"]
    test_pred = preds["Test"]["y_pred"]
    scatter_path = output_dir / "prediction_scatter_pollution.png"
    timeseries_path = output_dir / "timeseries_pollution.png"
    error_dist_path = output_dir / "error_distribution_pollution.png"

    plot_scatter(test_true, test_pred, scatter_path)
    plot_timeseries(test_true, test_pred, timeseries_path)
    plot_error_distribution(test_true, test_pred, error_dist_path)
