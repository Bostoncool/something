"""
北京PM2.5浓度预测 - Transformer模型 (GPU优化版本)
使用时间序列Transformer进行多步预测

GPU优化特性:
- 强制GPU检查和详细信息显示
- 混合精度训练（FP16）
- 优化数据加载（pin_memory, 多workers）
- GPU显存监控和自动清理
- 增大batch size提升训练效率
- 梯度裁剪防止梯度爆炸
...
# ============================== GPU优化配置 ==============================
print("=" * 80)
print("北京PM2.5浓度预测 - Transformer模型 (GPU优化版本)")
print("=" * 80)

# 强制GPU检查
if not torch.cuda.is_available():
    print("\n" + "❌" * 40)
    print("错误: 未检测到CUDA/GPU设备！")
    print("=" * 80)
    print("\n可能的原因:")
    ...
    print("\n解决方案:")
    print("  1. 检查GPU: 运行 'nvidia-smi' 命令")
    print("  2. 安装CUDA版本的PyTorch:")
    print("     pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu118")
    print("  3. 如果要使用CPU版本，请运行原始的 Transformer.py")
    print("=" * 80)
    import sys
    sys.exit(1)
...
if compute_capability[0] >= 7:
    print(f"  ✓ 支持混合精度训练（Tensor Cores）")
    USE_AMP = True
else:
    print(f"  ⚠️ 不完全支持混合精度训练（建议使用计算能力7.0+的GPU）")
    USE_AMP = False
...
# Transformer特定参数 - GPU优化
SEQ_LENGTH = 30  # 输入序列长度（30天）
PRED_LENGTH = 7  # 预测长度（7天）
BATCH_SIZE = 128  # 批量大小（GPU优化：从32增加到128）

# 根据显存大小动态调整batch size建议
if gpu_memory >= 8:
    recommended_batch_size = 128
    print(f"  ✓ 显存充足（{gpu_memory:.1f}GB），建议batch_size: {recommended_batch_size}")
elif gpu_memory >= 6:
    recommended_batch_size = 64
    print(f"  ⚠️ 显存适中（{gpu_memory:.1f}GB），建议batch_size: {recommended_batch_size}")
    if BATCH_SIZE > recommended_batch_size:
        print(f"  ⚠️ 当前batch_size({BATCH_SIZE})可能导致显存不足，建议降低")
else:
    recommended_batch_size = 32
    print(f"  ⚠️ 显存有限（{gpu_memory:.1f}GB），建议batch_size: {recommended_batch_size}")
    if BATCH_SIZE > recommended_batch_size:
        print(f"  ⚠️ 当前batch_size({BATCH_SIZE})可能导致显存不足，建议降低")
...
train_loader = DataLoader(
    ...
    shuffle=False,
    num_workers=4,  # GPU优化：多进程加载
    pin_memory=True,  # GPU优化：固定内存
    persistent_workers=True,  # GPU优化：持久化workers
    prefetch_factor=2  # GPU优化：预取因子
)
...
print(f"\n数据加载优化:")
print(f"  num_workers: 4 (异步数据加载)")
print(f"  pin_memory: True (加速数据传输到GPU)")
print(f"  persistent_workers: True (复用workers进程)")
print(f"  prefetch_factor: 2 (预加载2个batch)")
...
def train_model(model, train_loader, val_loader, epochs=100, lr=0.001, patience=50, verbose=True):
    ...
    for epoch in range(epochs):
        ...
        for batch_X, batch_y in train_loader:
            ...
            optimizer.zero_grad(set_to_none=True)  # GPU优化：更高效的梯度清零
        ...
        if verbose and (epoch + 1) % 10 == 0:
            ...
            print(f"  Epoch {epoch+1}/{epochs} - 训练损失: {train_loss:.6f}, 验证损失: {val_loss:.6f}, "
                  f"速度: {samples_per_sec:.1f} samples/sec, 时间: {epoch_time:.2f}s")
            print_gpu_memory("    ")
    ...
    if verbose and total_time > 0:
        ...
        print(f"\n训练统计:")
        print(f"  平均速度: {avg_samples_per_sec:.1f} samples/sec")
...
# ============================== 第8部分: 基础模型训练 ==============================
print("\n" + "=" * 80)
print("第4步: Transformer基础模型训练（GPU加速）")
print("=" * 80)
...
axes[0].set_title('Transformer基础模型(GPU) - 训练过程', fontsize=13, fontweight='bold')
...
axes[1].set_title('Transformer优化模型(GPU) - 训练过程', fontsize=13, fontweight='bold')
axes[1].legend(fontsize=10)
axes[1].grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(output_dir / 'training_curves_gpu.png', dpi=300, bbox_inches='tight')
print("保存: training_curves_gpu.png")
plt.close()
...
axes[0].set_title('Transformer基础模型(GPU) - 时间序列预测对比（测试集最后300个样本，第1天预测）',
...
axes[1].set_title('Transformer优化模型(GPU) - 时间序列预测对比（测试集最后300个样本，第1天预测）',
                  fontsize=13, fontweight='bold')
axes[1].legend(fontsize=10)
axes[1].grid(True, alpha=0.3)
...
print("保存: feature_importance_gpu.png")
plt.close()
...
print("保存: feature_importance_gpu.csv")
...
print("保存: transformer_optimized_full_gpu.pth")
...
print("保存: transformer_optimized_gpu.pkl")
...
print("  - feature_importance_gpu.csv      特征重要性")
print("  - best_parameters_gpu.csv         最佳参数")
print("  - predictions_gpu.csv             预测结果")

print("\n图表文件:")
print("  - training_curves_gpu.png         训练过程曲线")
...
print("  - feature_importance_gpu.png      特征重要性图")
print("  - model_comparison_gpu.png        模型性能对比")
print("  - error_distribution_gpu.png      误差分布")

print("\n模型文件:")
print("  - transformer_optimized_gpu.pth       Transformer模型（权重）")
print("  - transformer_optimized_full_gpu.pth  Transformer模型（完整）")
print("  - transformer_optimized_gpu.pkl       Transformer模型（pickle格式）")
...
print(f"  Pin Memory: 启用")
print(f"  梯度裁剪: 启用")

print_gpu_memory("\n最终 ")

print("\n" + "=" * 80)
print("Transformer PM2.5浓度预测完成！(GPU优化版本)")
print("=" * 80)
