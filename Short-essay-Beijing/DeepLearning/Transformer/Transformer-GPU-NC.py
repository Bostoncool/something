...
BATCH_SIZE = 512  # Batch size (GPU optimized for 32GB: increased from 128 to 512)
...
def read_era5_file(file_path, year, month):
    """Read a single ERA5 NetCDF file and extract all data variables."""
    start_day = 1
...
def read_era5_month(year, month):
    """Read all ERA5 NetCDF files for a single month and merge by variable."""
    month_str = f"{year}{month:02d}"
    
    # 递归搜索所有.nc文件（优先匹配月份，否则搜索所有文件）
    all_files = glob.glob(os.path.join(era5_path, "**", f"*{month_str}*.nc"), recursive=True)
    if not all_files:
        # 如果没有找到匹配月份的文件，搜索所有文件（用于fallback）
    ...
    # 按变量分组存储数据
    var_datasets = {}  # {var_name: [list of datasets]}
    
    files_read = 0
    files_skipped = 0
    
    for file_path in all_files:
        ds_result = read_era5_file(file_path, year, month)
        if ds_result is not None:
            ...
            for var_name in ds_result.data_vars:
                ...
                # 只保留该变量的数据
                var_ds = ds_result[[var_name]]
                var_datasets[var_name].append(var_ds)
            files_read += 1
        else:
            files_skipped += 1
    ...
    try:
        ...
        print(f"  Successfully read: {year}-{month:02d}, files: {files_read}/{len(all_files)}, "
              f"days: {len(df_month)}, variables: {len(df_month.columns)}")
        return df_month
    except Exception as exc:
        return None

def read_era5_file_universal(args):
    """Read a single ERA5 NetCDF file without month restriction, extract all data variables (multiprocessing compatible)."""
...
def read_all_era5():
    """Read all ERA5 data from NetCDF files in parallel using multiprocessing, group by variable and align by time."""
    ...
    # 并行读取所有文件
    print(f"\nReading {len(all_nc)} files in parallel...")
    var_datasets = {}  # {var_name: [list of datasets]}
    files_read = 0
    files_failed = 0
    
    # Prepare arguments for multiprocessing
    args_list = [(file_path, start_date, end_date, beijing_lats, beijing_lons) 
                  for file_path in all_nc]
    
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {executor.submit(read_era5_file_universal, args): args[0] 
                  for args in args_list}
        
        if TQDM_AVAILABLE:
            for future in tqdm(as_completed(futures), total=len(futures), 
                             desc="Reading NetCDF files", unit="file"):
            ...
            for i, future in enumerate(as_completed(futures), 1):
                ...
                if i % 100 == 0 or i == len(futures):
                    print(f"  Progress: {i}/{len(futures)} files (success: {files_read}, failed: {files_failed}, {i/len(futures)*100:.1f}%)")
    
    print(f"\nFile reading summary:")
    print(f"  Successfully read: {files_read}/{len(all_nc)} files")
    print(f"  Failed: {files_failed}/{len(all_nc)} files")
    print(f"  Variables found: {len(var_datasets)}")
    
    if not var_datasets:
        print("\nError: No data variables were extracted from files!")
        return pd.DataFrame()
    ...
    for var_name, datasets in var_datasets.items():
        if datasets:
            try:
                ...
                var_merged = var_merged.groupby('time').mean()
                merged_datasets.append(var_merged)
                print(f"  [+] {var_name}: {len(datasets)} files -> {var_merged.sizes.get('time', 0)} time points")
            except Exception as exc:
                print(f"  [-] Failed to merge {var_name}: {exc}")
                continue
    
    if not merged_datasets:
        print("\nError: Failed to merge any variables!")
        return pd.DataFrame()
...
optimal_workers = min(8, max(4, CPU_COUNT // 4))  # Use 4-8 workers
optimal_prefetch = 4  # Increase prefetch for better pipeline
...
model_basic, train_losses_basic, val_losses_basic, epochs_trained_basic = train_model(
    ...
    gradient_accumulation_steps=1  # No gradient accumulation for basic model
)
...
gradient_accum_steps = max(1, 512 // BATCH_SIZE)  # Target effective batch size of 512
