...
def plot_scatter(y_true, y_pred, stage):
    ...
    plt.scatter(y_true, y_pred, alpha=0.5, edgecolors="k", linewidths=0.2)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / f"prediction_scatter_{stage.lower()}.png", dpi=300)
    plt.close()


def plot_timeseries(idx, y_true, y_pred, stage, window=300):
    plt.figure(figsize=(12, 5))
    time_idx = pd.DatetimeIndex(idx)[-window:]
    plt.plot(time_idx, y_true[-window:], label="Actual", linewidth=1.0)
    plt.plot(time_idx, y_pred[-window:], label="Prediction", linewidth=1.0, linestyle='--')
    plt.xlabel("Date")
    plt.ylabel("PM2.5 (µg/m³)")
    plt.title(f"Time Series Comparison - {stage} (last {len(time_idx)} days)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / f"timeseries_{stage.lower()}.png", dpi=300)
    plt.close()


def plot_residuals(y_true, y_pred, stage):
    residuals = y_true - y_pred
    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    sns.histplot(residuals, bins=40, kde=True, ax=axes[0], color="steelblue")
    axes[0].axvline(0, color="r", linestyle="--")
    axes[0].set_title(f"Residual Distribution - {stage}")
    axes[0].set_xlabel("Residual (µg/m³)")

    axes[1].scatter(y_pred, residuals, alpha=0.5, edgecolors="k", linewidths=0.2)
    axes[1].axhline(0, color="r", linestyle="--")
    axes[1].set_xlabel("Predicted (µg/m³)")
    axes[1].set_ylabel("Residual (µg/m³)")
    axes[1].set_title(f"Residual vs Prediction - {stage}")

    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / f"residuals_{stage.lower()}.png", dpi=300)
    plt.close()
...
def main():
    ...
    for stage in ["train", "val", "test"]:
        ...
        plot_residuals(tensors[f"y_{stage}"], eval_result["pred"], stage)
