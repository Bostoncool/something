...
==================================================================
This script loads pollutant observations (CSV) together with ERA5 meteorological
features stored as NetCDF (.nc) files—following the LightGBM NetCDF ingestion
pipeline—and trains SVR models to forecast PM2.5 concentration.
"""
...
def find_file(base_path: str, date_str: str, prefix: str):
    """Locate CSV file that matches prefix_YYYYMMDD.csv."""
...
def read_all_era5():
    """Read all ERA5 data in parallel from NetCDF files."""
...
axes[0, 1].set_title(f'Test Set Prediction Results (R²={best_test_r2:.4f})', fontsize=12, fontweight='bold')
axes[0, 1].legend()
