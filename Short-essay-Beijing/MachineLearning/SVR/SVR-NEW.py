...
if __name__ == '__main__':
    ...
    for name, model in models.items():
        ...
        # Permutation importance on Test set (works for any SVR kernel)
        print("\nComputing permutation importance on Test set (neg MSE on scaled y)...")
        ...
        permutation_importance_by_model[name] = perm_df

        # per-model CSV
        perm_out = output_dir / f"permutation_importance__{name}__test.csv"
        save_csv(perm_df, perm_out)
        ...
        for _, row in perm_df.iterrows():
            permutation_importance_rows.append({
                "Model": name,
                "Dataset": "Test",
                "Feature": row["feature"],
                "Importance Mean": float(row["importance_mean"]),
                "Importance Std": float(row["importance_std"]),
            })
    ...
    # Combined permutation importance (Test set)
    if permutation_importance_rows:
        perm_all_df = pd.DataFrame(permutation_importance_rows)
        save_csv(perm_all_df, output_dir / "permutation_importance__all_models__test.csv")
