...
def plot_training_curves(
    ...
    def _plot_curve(evals, model, title: str, filename: str):
        ...
        plt.tight_layout()
        plt.savefig(output_dir / filename, dpi=300, bbox_inches="tight", format="tif")
        plt.close()

    _plot_curve(evals_result, model_basic, "XGBoost Basic Model - Training Process", "training_curve_basic.tif")
    _plot_curve(evals_result_opt, model_opt, "XGBoost Optimized Model - Training Process", "training_curve_optimized.tif")
...
def plot_prediction_scatter(
    ...
    for idx, (model_name, y_pred, y_true, dataset) in enumerate(models_data):
        ...
        ax.scatter(
            ...
            edgecolors="black",
            linewidth=0.3,
            facecolors="blue",  # 使用matplotlib默认蓝色
        )
        # 理想预测线：红色虚线，线宽2
        ...
        plt.tight_layout()
        plt.savefig(output_dir / f"prediction_scatter_{model_name}_{dataset}.tif", dpi=300, bbox_inches="tight", format="tif")
        plt.close()
...
def plot_time_series(
    ...
    # 合并时序图：在同一图中显示实际值、基础模型预测值和优化模型预测值
    fig, ax = plt.subplots(1, 1, figsize=(18, 5))
    # 实际值：黑色实线，线宽2
    ax.plot(time_idx, y_test.iloc[plot_idx], color="black", linestyle="-", label="Actual", linewidth=2)
    # 基础模型预测值：蓝色虚线，线宽1.5
    ax.plot(
        time_idx,
    ...
    ax.set_title(
        "XGBoost Model Comparison - Last 300 Days Prediction",
        fontsize=13,
        fontweight="bold",
    )
    ax.legend(fontsize=10)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "timeseries_comparison.tif", dpi=300, bbox_inches="tight", format="tif")
    plt.close()
...
def plot_residuals(
    ...
    for idx, (model_name, y_pred, y_true, dataset) in enumerate(models_data):
        ...
        # 残差图设置：与散点图保持一致，matplotlib默认蓝色填充，黑色边缘
        ax.scatter(
            y_pred,
            residuals,
            alpha=0.5,
            ...
            edgecolors="black",
            linewidth=0.3,
            facecolors="blue",  # 使用matplotlib默认蓝色，与散点图保持一致
        )
        # 零残差线：红色虚线，线宽2（与散点图的理想预测线保持一致）
        ...
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(output_dir / f"residuals_{model_name}_{dataset}.tif", dpi=300, bbox_inches="tight", format="tif")
        plt.close()
...
def main():
    ...
    print("\nChart files:")
    print("  - training_curve_basic.tif / training_curve_optimized.tif    Training curves")
    print("  - prediction_scatter_*.tif      Prediction vs Actual scatter plot")
    print("  - timeseries_comparison.tif     Time series prediction comparison")
    print("  - residuals_*.tif                Residual analysis")
