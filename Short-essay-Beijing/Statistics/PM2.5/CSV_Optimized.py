...
class PM25Analyzer:
    ...
    def plot_timeseries(self, save_path='pm25_timeseries.png'):
        ...
        if len(self.pm25_data) > 24:
            ...
            ax.plot(self.pm25_data['datetime'], rolling_mean, 
                    linewidth=1, color='#e74c3c', alpha=0.6, label='24-hour Moving Average', linestyle='--')
    ...
    def plot_aqi_distribution(self, save_path='aqi_distribution.png'):
        ...
        # Collect all text objects (labels and percentages)
        all_texts = list(texts) + list(autotexts)
        # Adjust text positions to avoid overlap
        # 使用 adjust_text 自动调整文本位置，避免重叠
        adjust_text(all_texts, ax=ax, 
                   arrowprops=dict(arrowstyle='->', color='gray', lw=0.5, alpha=0.6),
                   force_text=(0.3, 0.3),  # 文本之间的排斥力
                   force_static=(0.1, 0.1),  # 静态排斥力
                   expand_text=(1.1, 1.1),  # 文本扩展范围
                   expand_points=(1.1, 1.1),  # 点扩展范围
                   lim=500)  # 最大迭代次数
