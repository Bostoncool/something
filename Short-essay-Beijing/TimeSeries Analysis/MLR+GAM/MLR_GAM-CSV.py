...
# ---------------------------------------------------------------------------
# Output helpers (strictly follow RF-CSV naming style + UTF-8-SIG)
# ---------------------------------------------------------------------------
...
def evaluate_metrics(y_true: pd.Series, y_pred: np.ndarray, model_name: str, dataset: str):
    """统一指标输出（与 RF-CSV 风格一致）。"""
    y_pred = np.asarray(y_pred)
    r2 = float(r2_score(y_true, y_pred))
    rmse = float(np.sqrt(mean_squared_error(y_true, y_pred)))
    mae = float(mean_absolute_error(y_true, y_pred))
    denom = np.where(np.asarray(y_true) == 0, np.nan, np.asarray(y_true))
    mape = float(np.nanmean(np.abs((np.asarray(y_true) - y_pred) / denom)) * 100)
    return {
        'Model': model_name,
        'Dataset': dataset,
        'R²': r2,
...
if __name__ == '__main__':
    ...
    # ============================== Part10/11: Output CSV (no plots, no model files) ==============================
    print("\n" + "=" * 80)
    print("Exporting CSVs for downstream plotting (no figures/model files will be generated)")
    print("=" * 80)

    # ---- Metrics (Train/Test; split like RF-CSV) ----
    metrics_rows = [
        evaluate_metrics(y_train, y_train_pred_mlr, 'MLR', 'Train'),
        evaluate_metrics(y_test, y_test_pred_mlr, 'MLR', 'Test'),
        evaluate_metrics(y_train, y_train_pred_gam, 'GAM', 'Train'),
        evaluate_metrics(y_test, y_test_pred_gam, 'GAM', 'Test'),
    ]
    metrics_df = pd.DataFrame(metrics_rows)

    save_csv(metrics_df, output_dir / 'metrics__all_models_train_test.csv')
    for model in metrics_df['Model'].unique():
        model_slug = str(model).lower()
        save_csv(metrics_df[metrics_df['Model'] == model], output_dir / f'metrics__{model_slug}__train_test.csv')
        for ds_name in metrics_df['Dataset'].unique():
            part = metrics_df[(metrics_df['Model'] == model) & (metrics_df['Dataset'] == ds_name)]
            if not part.empty:
                save_csv(part, output_dir / f'metrics__{model_slug}__{str(ds_name).lower()}.csv')

    test_results = metrics_df[metrics_df['Dataset'] == 'Test'].sort_values('R²', ascending=False).reset_index(drop=True)
    save_csv(test_results, output_dir / 'plot_metrics_ranking__test_only.csv')

    # ---- Predictions (split by model) ----
    pred_mlr_df = pd.DataFrame({
        'Date': y_test.index,
        'Actual_PM25': y_test.values,
        'Predicted_PM25': np.asarray(y_test_pred_mlr),
        'Error': (y_test.values - np.asarray(y_test_pred_mlr)),
    })
    save_csv(pred_mlr_df, output_dir / 'predictions__mlr.csv')

    pred_gam_df = pd.DataFrame({
        'Date': y_test.index,
        'Actual_PM25': y_test.values,
        'Predicted_PM25': np.asarray(y_test_pred_gam),
        'Error': (y_test.values - np.asarray(y_test_pred_gam)),
    })
    save_csv(pred_gam_df, output_dir / 'predictions__gam.csv')

    pred_all_df = pred_mlr_df[['Date', 'Actual_PM25']].copy()
    pred_all_df = pred_all_df.assign(
        Predicted_MLR=np.asarray(y_test_pred_mlr),
        Predicted_GAM=np.asarray(y_test_pred_gam),
        Error_MLR=pred_mlr_df['Error'].values,
        Error_GAM=pred_gam_df['Error'].values,
    )
    save_csv(pred_all_df, output_dir / 'predictions__all_models.csv')

    # ---- Plot data: simple time series (按模型拆分；参考 RF 的 plot_ts_simple_sampled__) ----
    plot_df_simple = pd.DataFrame({
        'time': y_test.index,
        'y_true': y_test.values,
        'y_pred_mlr': np.asarray(y_test_pred_mlr),
        'y_pred_gam': np.asarray(y_test_pred_gam),
    }).sort_values('time').reset_index(drop=True)

    step_simple = 4
    plot_df_simple_sampled = plot_df_simple.iloc[::step_simple].copy()

    save_csv(
        plot_df_simple_sampled[['time', 'y_true', 'y_pred_mlr']].rename(columns={'y_pred_mlr': 'y_pred'}),
        output_dir / 'plot_ts_simple_sampled__mlr.csv'
    )
    save_csv(
        plot_df_simple_sampled[['time', 'y_true', 'y_pred_gam']].rename(columns={'y_pred_gam': 'y_pred'}),
        output_dir / 'plot_ts_simple_sampled__gam.csv'
    )

    # ---- Plot data: scatter (按模型拆分) ----
    for model_name, pred in [('mlr', y_test_pred_mlr), ('gam', y_test_pred_gam)]:
        scatter_df = pd.DataFrame({
            'Date': y_test.index,
            'Actual_PM25': y_test.values,
            'Predicted_PM25': np.asarray(pred),
        })
        save_csv(scatter_df, output_dir / f'plot_scatter__{model_name}.csv')

    # ---- Plot data: residuals / error distribution (按模型拆分) ----
    for model_name, pred in [('mlr', y_test_pred_mlr), ('gam', y_test_pred_gam)]:
        pred_arr = np.asarray(pred)
        residuals = y_test.values - pred_arr

        residuals_df = pd.DataFrame({
            'Date': y_test.index,
            'Actual_PM25': y_test.values,
            'Predicted_PM25': pred_arr,
            'Residual': residuals,
        })
    ...
    # ---- Plot data: time series (按模型拆分；参考 RF 的 “sampled + x_axis”) ----
    ...
    plot_range = 200
    y_true_plot = y_test_sorted.iloc[-plot_range:].copy()
    y_mlr_plot = y_test_pred_mlr_sorted.iloc[-plot_range:].copy()
    y_gam_plot = y_test_pred_gam_sorted.iloc[-plot_range:].copy()

    plot_df = pd.DataFrame({
        'time': y_true_plot.index,
        'y_true': y_true_plot.values,
        'y_pred_mlr': y_mlr_plot.values,
        'y_pred_gam': y_gam_plot.values,
    }).sort_values('time').reset_index(drop=True)

    step = 1 if len(plot_df) <= 250 else 4
    plot_df_sampled = plot_df.iloc[::step].copy()
    ...
    save_csv(ts_common, output_dir / f'plot_ts_last{plot_range}_sampled__actual.csv')

    ts_mlr = ts_common.assign(y_pred=plot_df_sampled['y_pred_mlr'].values)
    save_csv(ts_mlr, output_dir / f'plot_ts_last{plot_range}_sampled__mlr.csv')

    ts_gam = ts_common.assign(y_pred=plot_df_sampled['y_pred_gam'].values)
    save_csv(ts_gam, output_dir / f'plot_ts_last{plot_range}_sampled__gam.csv')

    # ---- Feature importance: MLR coefficients (按模型拆分 + TopN) ----
    mlr_coef = np.asarray(mlr_pipeline.named_steps['regressor'].coef_).ravel()
    fi_mlr = (
        pd.DataFrame({'Feature': selected_features, 'Coefficient': mlr_coef})
        .assign(Abs_Coefficient=lambda d: np.abs(d['Coefficient']))
        .sort_values('Abs_Coefficient', ascending=False)
        .reset_index(drop=True)
    )
    save_csv(fi_mlr.drop(columns=['Abs_Coefficient']), output_dir / 'plot_feature_importance__mlr.csv')
    top_n = min(15, len(fi_mlr))
    save_csv(fi_mlr.head(top_n).drop(columns=['Abs_Coefficient']), output_dir / f'plot_feature_importance_top{top_n}__mlr.csv')

    # ---- Feature importance: GAM term p-values (更合理避免“系数=特征”的误映射) ----
    gam_pvals = None
    try:
        gam_pvals = getattr(gam, 'statistics_', {}).get('p_values', None)
    except Exception:
        gam_pvals = None

    if gam_pvals is not None and len(gam_pvals) >= (len(selected_features) + 1):
        fi_gam = pd.DataFrame({
            'Feature': selected_features,
            'PValue': np.asarray(gam_pvals[1:len(selected_features) + 1], dtype=float),
        }).sort_values('PValue', ascending=True).reset_index(drop=True)
    else:
        fi_gam = pd.DataFrame({'Feature': selected_features, 'PValue': np.nan})

    save_csv(fi_gam, output_dir / 'plot_feature_importance__gam.csv')
    top_n_gam = min(15, len(fi_gam))
    save_csv(fi_gam.head(top_n_gam), output_dir / f'plot_feature_importance_top{top_n_gam}__gam.csv')
