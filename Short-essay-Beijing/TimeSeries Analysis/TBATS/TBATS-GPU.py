...
# Seasonal patterns for TBATS (Beijing PM2.5 may have multiple seasonal patterns)
SEASONAL_PERIODS = [7, 30, 365]  # Daily, monthly, yearly seasonality

print(f"Data time range: {start_date.date()} to {end_date.date()}")
print(f"Target variable: PM2.5 concentration")
print(f"TBATS seasonal periods: {SEASONAL_PERIODS} days")
...
def grid_search_tbats(train_data, val_data, seasonal_periods_options=None, verbose=False):
    ...
    if seasonal_periods_options is None:
        seasonal_periods_options = [
            [7], [30], [365],  # Single seasonality
            [7, 30], [7, 365], [30, 365],  # Double seasonality
            [7, 30, 365]  # Triple seasonality
        ]
