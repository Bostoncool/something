...
class NCToCSVConverter:
    ...
    def handle_missing_values(self, df, ds):
        ...
        """
        # 检查u10变量的缺失值
        if 'u10' in ds.variables and '_FillValue' in ds['u10'].attrs:
            fill_value = ds['u10'].attrs['_FillValue']
            df['u10'] = df['u10'].replace(fill_value, np.nan)
    ...
    def save_to_csv(self, df, output_file, ds):
        ...
        # 添加单位信息作为注释
        units_info = []
        if 'u10' in ds.variables and 'units' in ds['u10'].attrs:
            units_info.append(f"u10 units: {ds['u10'].attrs['units']}")
...
def main():
    ...
    """
    # 在这里指定您的文件夹路径
    input_folder = r"C:\Users\IU\Desktop\Datebase Origin\ERA5-Beijing-NC\10m_u_component_of_wind"  # 替换为您的输入文件夹路径
    output_folder = r"C:\Users\IU\Desktop\Datebase Origin\ERA5-Beijing-CSV\10m_u_component_of_wind"  # 替换为您的输出文件夹路径
