...
class NCToCSVConverter:
    ...
    def create_dataframe_from_nc(self, ds):
        ...
        # 提取v100数据并展平
        v100_data = ds['v100'].values
        v100_flat = v100_data.reshape(-1)
    ...
    def handle_missing_values(self, df, ds):
        ...
        """
        # 检查v100变量的缺失值
        if 'v100' in ds.variables and '_FillValue' in ds['v100'].attrs:
            fill_value = ds['v100'].attrs['_FillValue']
            df['v100'] = df['v100'].replace(fill_value, np.nan)
    ...
    def save_to_csv(self, df, output_file, ds):
        ...
        # 添加单位信息作为注释
        units_info = []
        if 'v100' in ds.variables and 'units' in ds['v100'].attrs:
            units_info.append(f"v100 units: {ds['v100'].attrs['units']}")
...
def main():
    ...
    """
    # 在这里指定您的文件夹路径
    input_folder = r"C:\Users\IU\Desktop\Datebase Origin\ERA5-Beijing-NC\100m_v_component_of_wind"  # 替换为您的输入文件夹路径
    output_folder = r"C:\Users\IU\Desktop\Datebase Origin\ERA5-Beijing-CSV\100m_v_component_of_wind"  # 替换为您的输出文件夹路径
