...
DEFAULT_INPUT_DIR = _DATA_ROOT / "2018-2023[ERA5_PM2.5-clusters]"
DEFAULT_OUTPUT_LONG_CSV = _DATA_ROOT / "era5_yearly_summary_long_2018_2023.csv"
DEFAULT_OUTPUT_WIDE_CSV = _DATA_ROOT / "era5_yearly_summary_wide_2018_2023.csv"
...
def infer_meteorology_folder_name(file_path: Path) -> str:
    known_folders = {
...
def sample_one_per_folder(nc_files: list[Path]) -> list[Path]:
    ...
    for path in nc_files:
        folder = infer_meteorology_folder_name(path)
        if folder not in seen:
            seen[folder] = path
    return sorted(seen.values(), key=lambda p: (infer_meteorology_folder_name(p), str(p)))
...
def build_yearly_records(file_path_str: str) -> list[YearlyRecord]:
    ...
    folder_name = infer_meteorology_folder_name(file_path)
