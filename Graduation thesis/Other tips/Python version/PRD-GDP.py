"""
珠三角地区 GDP 分布图 - Python 复现
基于 R 代码 PRD-GDP.R 的功能实现
"""
...
LAEA_CRS = CRS.from_proj4(
    "+proj=laea +lat_0=23.1 +lon_0=113.2 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs"
)
WGS84 = CRS.from_epsg(4326)
BASE_PATH = Path("H:/大论文Result/大论文图")
OUTPUT_SVG = "珠三角_GDP_2024.svg"
OUTPUT_PNG = "珠三角_GDP_2024.png"
MISSING_OUTPUT = "珠三角_缺失GDP数据区域.csv"
...
def clean_gdp_data(file_path: Path) -> pd.DataFrame:
    """清理广东省 GDP 数据并筛选珠三角城市"""
    ...
    gdp_col = columns[5]
    cleaned = (
        df.loc[(df["年份"] == 2024) & (df["level"] == 2), [region_col, city_col, gdp_col]]
        .rename(columns={region_col: "区域名称", city_col: "城市", gdp_col: "GDP_2024"})
        .assign(省份="广东省")
    )
    cleaned["GDP_2024"] = pd.to_numeric(cleaned["GDP_2024"], errors="coerce")
    return cleaned.loc[
        cleaned["GDP_2024"].notna() & (cleaned["GDP_2024"] > 0) & cleaned["城市"].isin(PRD_CITIES)
    ].copy()
...
def main() -> None:
    ...
    gdp_data = clean_gdp_data(BASE_PATH / "三大城市群的具体GDP/广东省各区GDP（2023-2024）.csv")
    ...
    geo_frames = []
    for city_name, level in city_geo_config:
        gdf = read_and_transform(BASE_PATH / f"4.珠三角/具体城市（区分辨率）/{city_name} (区).geojson")
        gdf = gdf.loc[gdf["level"] == level, ["name", "geometry"]].assign(城市=city_name)
        geo_frames.append(gdf)
    geo_data = pd.concat(geo_frames, ignore_index=True)
    merged_data = gpd.GeoDataFrame(
        geo_data.merge(gdp_data, left_on=["name", "城市"], right_on=["区域名称", "城市"], how="left"),
        geometry="geometry",
        crs=LAEA_CRS,
    )
    ...
    ax.set_title("PRD 2024 GDP Distribution", fontsize=32, fontweight="bold")
    ...
    missing = (
        merged_data.loc[merged_data["GDP_2024"].isna(), ["name"]]
        .assign(省份="广东省")
        .sort_values(["省份", "name"])
