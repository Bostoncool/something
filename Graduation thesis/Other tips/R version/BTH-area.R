...
# 设置LAEA投影（局部等面积投影），中心点设置为京津冀地区中心（经度116°，纬度39°）
...
# 定义京津冀地区分区边界文件配置 - 用于绘制省界轮廓
province_config <- list(
  files = c(
    "C:/Users/IU/Desktop/大论文图/2.京津冀/北京市 (划分).geojson",
    "C:/Users/IU/Desktop/大论文图/2.京津冀/天津市 (划分).geojson",
    "C:/Users/IU/Desktop/大论文图/2.京津冀/河北省 (市).geojson"
  )
)

# 定义京津冀地区的具体市区文件配置 - 包含所有具体的市区行政区划
bth_config <- list(
  files = c(
    "C:/Users/IU/Desktop/大论文图/2.京津冀/北京市 (划分).geojson",
    "C:/Users/IU/Desktop/大论文图/2.京津冀/天津市 (划分).geojson",
...
# 读取京津冀分区数据
bth_shapes <- map(bth_config$files, read_and_transform)

# 为每个市区数据添加区域名称列，只保留geometry和region列
# 确保只保留具体的市区级别的行政区划数据
bth_shapes_with_region <- map(bth_shapes, function(shp) {
  ...
  shp_simple <- st_sf(region = "京津冀地区", geometry = st_geometry(shp_filtered))
  return(shp_simple)
})

# 合并京津冀所有分区数据
bth_data <- do.call(rbind, bth_shapes_with_region)

# 获取京津冀地区的边界框，用于设置合适的显示范围
bth_bbox <- st_bbox(bth_data)

# 创建经纬度网格线（graticule）
# 获取地图边界框（在WGS84坐标系中）
bth_bbox_wgs84 <- st_bbox(bth_data %>% st_transform(crs = st_crs(4326)))

# 创建经纬度网格线，针对京津冀地区的经纬度范围
graticule <- st_graticule(
  bth_data %>% st_transform(crs = st_crs(4326)),
  lat = seq(floor(bth_bbox_wgs84$ymin), ceiling(bth_bbox_wgs84$ymax), by = 2),
  lon = seq(floor(bth_bbox_wgs84$xmin), ceiling(bth_bbox_wgs84$xmax), by = 2)
...
bth_map <- ggplot() +
  ...
  # 添加京津冀市区图层 - 绘制所有具体的市区行政区划
  geom_sf(data = bth_data, fill = bth_config$color, color = "gray", linewidth = 0.3)

# 添加主题、图例、比例尺、指北针和坐标
bth_final_map <- bth_map +
  # 设置填充颜色
  scale_fill_manual(
    name = "研究区域",
    values = c("京津冀地区" = bth_config$color),
    breaks = c("京津冀地区")
  ) +
  # 添加比例尺（右下角）
  ...
  coord_sf(
    crs = laea_crs,
    expand = FALSE,
    xlim = c(bth_bbox$xmin, bth_bbox$xmax),
    ylim = c(bth_bbox$ymin, bth_bbox$ymax),
