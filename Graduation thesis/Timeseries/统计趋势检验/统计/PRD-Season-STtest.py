...
def sliding_t_test(data, window_size=None):
    ...
    if window_size is None:
        window_size = max(4, n // 3)  # 季节数据更长，使用更大的窗口，至少4，至少为数据长度的1/3

    # 确保窗口大小合理
    window_size = min(window_size, n - 4)  # 留出至少4个季节用于另一半
    ...
    if significant and change_point > window_size and change_point < n - window_size:
        ...
        if len(before_data) >= 3:  # 季节数据至少需要3个季节来判断趋势
...
def process_single_file(file_path):
    """
    处理单个CSV文件，提取季节和城市污染物数据

    参数:
    file_path: CSV文件路径

    返回:
    (season, season_data) 元组，season_data是字典 {city: {pollutant: value}}
    """
    try:
        # 从文件名提取日期并转换为季节
        ...
        season_key = get_season_from_date(date_part)
        ...
        # 对于每个城市，存储PM2.5的季节均值（优先使用PM2.5_24h，如果没有则使用PM2.5）
...
def calculate_sliding_t_test(folder_path, window_size=None, n_processes=None):
    ...
    for result in results:
        if result is not None:
            season, season_data = result
            if season not in all_data:
                all_data[season] = {}
            all_data[season].update(season_data)

    # 整理数据，执行滑动T检验
    seasons = sorted(all_data.keys())
    print(f"处理了 {len(seasons)} 个季节的数据: {seasons[:5]}...{seasons[-5:] if len(seasons) > 5 else seasons}")
    ...
    for city in tqdm(all_cities, desc="执行滑动T检验", unit="城市"):
        ...
        # 至少需要8个季节的数据才能进行有意义的滑动T检验
        if len(city_pm25_data) >= 8:
            # 准备检验数据（按季节排序）
            ...
            if sliding_t_result['significant'] and sliding_t_result['change_point'] is not None:
                sliding_t_result['change_season'] = sorted_seasons[sliding_t_result['change_point']]
            ...
            sliding_t_results[city] = {
                ...
                'change_season': sliding_t_result['change_season'],  # 突变点季节
                ...
                'n_seasons': len(sorted_seasons)  # 数据季节数
            }
        else:
            # 数据不足，记录为NaN
            sliding_t_results[city] = {
    ...
    return sliding_t_results, seasons

def save_results_to_csv(sliding_t_results, seasons, output_dir):
    """
    将滑动T检验结果保存为CSV文件
    ...
    for city, result in tqdm(sliding_t_results.items(), desc="处理检验结果", unit="城市"):
        row = {
            ...
            'Change_Season': result['change_season'],  # 突变点季节
            ...
            'N_Seasons': result['n_seasons']  # 数据季节数
        }
    ...
    if sliding_t_data:
        ...
        sliding_t_path = os.path.join(output_dir, 'PRD_PM25_Seasonal_Sliding_T_Test.csv')
        df_sliding_t.to_csv(sliding_t_path, index=False, encoding='utf-8-sig')
        print(f"季节滑动T检验结果已保存到: {sliding_t_path}")

    print(f"\n所有结果已保存到目录: {output_dir}")

def print_summary(sliding_t_results, seasons):
    """
    打印滑动T检验结果摘要
    """
    print("\n" + "="*80)
    print("PM2.5 季节滑动T检验突变点分析摘要")
    print("="*80)
    ...
    for city, result in sliding_t_results.items():
        print(f"\n城市: {city}")
        print(f"  数据季节数: {result['n_seasons']}")
        print(f"  滑动窗口大小: {result['window_size']}")

        if result['significant']:
            print(f"  突变点位置: 第{result['change_point']}个观测点（{result['change_season']}）")
            ...
            seasons_list = result['seasons']
            values_list = result['values']

            # 显示突变点前后的几个季节
            start_idx = max(0, change_idx - 2)
            end_idx = min(len(seasons_list), change_idx + 3)
...
if __name__ == "__main__":
    ...
    # 输出目录路径
    output_dir = r"E:\DATA Science\大论文Result\PRD\统计趋势检验"
    ...
    if not os.path.exists(folder_path):
        ...
        # 执行滑动T检验（使用多进程）
        sliding_t_results, seasons = calculate_sliding_t_test(folder_path, window_size=window_size, n_processes=n_processes)

        # 打印摘要
        print_summary(sliding_t_results, seasons)

        # 保存结果为CSV文件
        save_results_to_csv(sliding_t_results, seasons, output_dir)

        print("\n季节滑动T检验完成！")
        print("\n说明:")
        print("- 季节滑动T检验通过比较滑动窗口内前后两段数据的均值差异来检测突变点")
        print("- p值 < 0.05 表示在该置信水平下存在显著突变点")
        print("- 突变点表示污染水平发生显著变化的时间点")
        print("- 趋势变化显示突变点前后污染水平的变动方向")
        print("- 季节分析对年际变化更敏感，可以更好地识别长期趋势变化")
        print("- 季节划分：3-5月为春，6-8月为夏，9-11月为秋，12-2月为冬")
        print("- 建议窗口大小至少为4个季节（1年）以获得稳定的结果")