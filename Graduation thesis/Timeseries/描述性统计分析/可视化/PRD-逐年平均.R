...
# =========================
geojson_files <- c(
  "E:/DATA Science/大论文Result/大论文图/4.珠三角/具体城市/广州市.geojson",
  "E:/DATA Science/大论文Result/大论文图/4.珠三角/具体城市/深圳市.geojson",
  "E:/DATA Science/大论文Result/大论文图/4.珠三角/具体城市/佛山市.geojson",
  "E:/DATA Science/大论文Result/大论文图/4.珠三角/具体城市/东莞市.geojson",
  "E:/DATA Science/大论文Result/大论文图/4.珠三角/具体城市/中山市.geojson",
  "E:/DATA Science/大论文Result/大论文图/4.珠三角/具体城市/珠海市.geojson",
  "E:/DATA Science/大论文Result/大论文图/4.珠三角/具体城市/惠州市.geojson",
  "E:/DATA Science/大论文Result/大论文图/4.珠三角/具体城市/江门市.geojson",
  "E:/DATA Science/大论文Result/大论文图/4.珠三角/具体城市/肇庆市.geojson"
)

pollution_dir <- "E:/DATA Science/大论文Result/PRD/filtered_daily"
output_dir <- "E:/DATA Science/大论文Result/大论文图/4.珠三角/PM2.5_年均_逐年"
...
normalize_city_name <- function(x) {
  ...
  case_when(
    str_detect(x_norm, "广州") ~ "广州",
    str_detect(x_norm, "深圳") ~ "深圳",
    str_detect(x_norm, "佛山") ~ "佛山",
    str_detect(x_norm, "东莞") ~ "东莞",
    str_detect(x_norm, "中山") ~ "中山",
    str_detect(x_norm, "珠海") ~ "珠海",
    str_detect(x_norm, "惠州") ~ "惠州",
    str_detect(x_norm, "江门") ~ "江门",
    str_detect(x_norm, "肇庆") ~ "肇庆",
    TRUE ~ x_norm
  )
}
...
for (yr in years_target) {
  ...
  p <- p +
    ...
    labs(
      title = paste0("珠三角PM2.5年均浓度分布（", yr, "）"),
      x = "经度 (°E)",
      y = "纬度 (°N)"
    ) +
    coord_sf(
  ...
  out_file <- file.path(output_dir, paste0("珠三角_PM2.5_年均_", yr, ".svg"))
  ggsave(
    filename = out_file,
    plot = p,
    device = "svg",
