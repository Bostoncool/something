"""
Daily PM2.5 prediction for BTH/YRD/PRD using K-Nearest Neighbors (KNN) regression with per-cluster training.
Reuses daily_ml_pipeline for data preparation and evaluation.
Note: File named K-Means.py but implements KNN (K-Nearest Neighbors) regression, not K-Means clustering.
"""
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "knn_daily_pm25"
...
def _get_knn_feature_importance(
    ...
    """KNN has no built-in feature importance; use permutation importance."""
...
def train_knn(
    ...
    if getattr(args, "enable_bayes_search", False):
        if BayesSearchCV is None or Real is None or Integer is None or Categorical is None:
            raise ImportError(
    ...
    model = KNeighborsRegressor(**train_params)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using K-Nearest Neighbors (KNN)."
    )
    parser.add_argument(
        "--daily-input",
        nargs="+",
    ...
    parser.add_argument(
        ...
        type=str,
        default="minkowski",
        help="Distance metric (minkowski, euclidean, manhattan, etc.).",
    )
    parser.add_argument(
        "--p",
        type=int,
        default=2,
        help="Power parameter for Minkowski metric (1=manhattan, 2=euclidean).",
    )
    parser.add_argument(
        "--early-stopping-rounds",
        type=int,
        default=0,
        help="Ignored (KNN has no early stopping); kept for API compatibility.",
    )
    parser.add_argument("--n-jobs", type=int, default=-1, help="Parallel jobs for neighbor search.")
    parser.add_argument(
        "--enable-bayes-search",
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="KNN 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    for cluster_name in clusters_to_train:
        ...
        try:
            (model, scaler_x), grid_search_info = train_knn(
    ...
    for cluster_name in target_cluster_order:
        ...
        joblib.dump(
            ...
            cluster_output_dir / "knn_best.joblib",
        )
        ...
        x_train_scaled = scaler_x.transform(x_train)
        importance = _get_knn_feature_importance(
            model,
            x_train_scaled,
            y_train.values,
            n_repeats=5,
    ...
    print("=" * 90)
    print("[INFO] KNN daily PM2.5 training finished.")
    ...
    print("       - run_info.json")
    print("       - knn_best.joblib (per cluster: model + scaler)")
    print("=" * 90)
    return 0
