...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="ST-Transformer 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    if getattr(args, "tune", False):
        ...
        def _objective(trial: "optuna.Trial") -> float:
            ...
            trial_args.fusion_dropout = trial.suggest_float("fusion_dropout", 0.05, 0.2)
            ...
            _, best_rmse = train_st_transformer(
                X_train_scaled_t,
                y_train_scaled_t,
                city_id_train_t,
                X_valid_scaled_t,
        ...
        study = optuna.create_study(
            ...
            sampler=sampler,
            pruner=pruner,
            study_name="st_transformer_daily_pm25",
        )
        study.optimize(
            _objective,
    ...
    for cluster_name in clusters_to_train:
        ...
        if int(getattr(args, "debug_overfit_n", 0)) > 0:
            ...
            if debug_n > 1:
                ...
                debug_model, _ = train_st_transformer(
                ...
                debug_pred_scaled = predict_numpy_st(
                    debug_model,
        ...
        pred_train_scaled = predict_numpy_st(model, X_train_scaled, city_id_train, device)
        pred_valid_scaled = predict_numpy_st(model, X_valid_scaled, city_id_valid, device)
        pred_test_scaled = predict_numpy_st(model, X_test_scaled, city_id_test, device)
    ...
    for cluster_name in target_cluster_order:
        ...
        export_regression_artifacts(all_pred_df=all_pred_df, output_dir=cluster_output_dir, model_name=f"ST_Transformer-{cluster_name}")
    ...
    export_regression_artifacts(all_pred_df=all_pred_df, output_dir=output_dir, model_name="ST_Transformer-ClusterModels")
    if stage_pbar is not None:
        stage_pbar.update(1)
