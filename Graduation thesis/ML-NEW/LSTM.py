"""
Daily PM2.5 prediction for BTH/YRD/PRD using LSTM-only with GPU/CPU.
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "lstm_daily_pm25"
...
class LSTMRegressor(nn.Module):
    """Pure LSTM regression: input (B, T, F) -> LSTM -> last timestep -> Dropout -> FC -> 1."""
    ...
    def __init__(
        ...
        self.fc = nn.Linear(lstm_hidden * hidden_mult, 1)
...
def train_lstm(
    ...
    if tqdm is not None:
        epoch_iter = tqdm(epoch_iter, desc="LSTM 训练轮次", dynamic_ncols=True)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using LSTM (GPU/CPU)."
    )
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="LSTM 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    for cluster_name in target_cluster_order:
        ...
        export_regression_artifacts(all_pred_df=all_pred_df, output_dir=cluster_output_dir, model_name=f"LSTM-{cluster_name}")
    ...
    export_regression_artifacts(all_pred_df=all_pred_df, output_dir=output_dir, model_name="LSTM-ClusterModels")
    if stage_pbar is not None:
        stage_pbar.update(1)
    ...
    print("=" * 90)
    print("[INFO] LSTM daily PM2.5 training finished.")
