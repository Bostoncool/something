"""
Daily PM2.5 prediction for BTH/YRD/PRD using a CNN+GRU hybrid sequence model with GPU/CPU.
...
from daily_ml_pipeline import (  # type: ignore[import]
...
from cluster_training_utils import prepare_training_table_with_fallback  # type: ignore[import]
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "cnn_gru_daily_pm25"
...
class CNNGRURegressor(nn.Module):
    """Hybrid CNN+GRU sequence regressor: (B, T, F) -> Conv1d over time -> GRU -> Dropout -> FC -> 1."""
    ...
    def __init__(
        ...
        cnn_kernel_sizes: tuple[int, ...] | None = None,
        cnn_pool_size: int = 2,
        gru_hidden: int = 128,
        gru_layers: int = 2,
        dropout: float = 0.3,
        ...
        if cnn_kernel_sizes is None:
            cnn_kernel_sizes = (3,) * len(cnn_channels)
        if len(cnn_kernel_sizes) != len(cnn_channels):
            cnn_kernel_sizes = (3,) * len(cnn_channels)
        ...
        for out_ch, k in zip(cnn_channels, cnn_kernel_sizes):
            pad = k // 2
            layers.append(nn.Conv1d(in_ch, out_ch, kernel_size=k, padding=pad))
            ...
            in_ch = out_ch
            L = (L + 2 * pad - k) // 1
            L = (L - cnn_pool_size) // cnn_pool_size + 1
        self.conv = nn.Sequential(*layers)
        self._conv_out_len = max(1, L)
        self._conv_out_ch = in_ch

        gru_input_size = self._conv_out_ch
        self.gru = nn.GRU(
        ...
        self.fc = nn.Linear(gru_hidden * hidden_mult, 1)

    def forward(self, x: "torch.Tensor") -> "torch.Tensor":
        # x: (B, T, F)
        x = x.transpose(1, 2)  # (B, F, T)
        h = self.conv(x)  # (B, C, T_conv)
        h = h.transpose(1, 2)  # (B, T_conv, C)
        out, _ = self.gru(h)
        last_step = out[:, -1, :]
...
def train_cnn_gru(
    ...
    channels = getattr(args, "cnn_channels", (64, 128))
    if isinstance(channels, (list, tuple)):
        channels_tuple: tuple[int, ...] = tuple(int(c) for c in channels)
    else:
        channels_tuple = (int(channels),)
    kernel_sizes = getattr(args, "cnn_kernel_sizes", None)
    if kernel_sizes is not None and isinstance(kernel_sizes, (list, tuple)):
        kernel_sizes_tuple: tuple[int, ...] | None = tuple(int(k) for k in kernel_sizes)
    else:
        kernel_sizes_tuple = None

    model = CNNGRURegressor(
        input_size=n_features,
        seq_len=seq_len,
        cnn_channels=channels_tuple,
        cnn_kernel_sizes=kernel_sizes_tuple,
        cnn_pool_size=int(getattr(args, "cnn_pool_size", 2)),
        gru_hidden=int(getattr(args, "gru_hidden", 128)),
        gru_layers=int(getattr(args, "gru_layers", 2)),
        dropout=float(getattr(args, "dropout", 0.3)),
        bidirectional=bool(getattr(args, "bidirectional", False)),
        seed=args.seed,
    ).to(device)

    loss_mode = str(getattr(args, "loss", "mse")).strip().lower()
    criterion = nn.MSELoss()
    if loss_mode == "smoothl1":
        criterion = nn.SmoothL1Loss(beta=float(getattr(args, "smoothl1_beta", 1.0)))
    elif loss_mode not in {"mse", "weighted_mse"}:
    ...
    scheduler_mode = str(getattr(args, "scheduler", "plateau")).strip().lower()
    if scheduler_mode == "plateau":
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            ...
            eta_min=float(getattr(args, "min_lr", 1e-6)),
        )
    else:
        raise ValueError(f"Unsupported scheduler mode: {scheduler_mode}")

    train_loader, valid_loader = build_sequence_dataloaders(
        X_train, y_train, X_valid, y_valid, args.batch_size, device, args.num_workers
    )
    best_rmse = float("inf")
    patience = getattr(args, "early_stopping_patience", 15)
    epochs_no_improve = 0
    best_state: dict[str, Any] | None = None
    amp_enabled = bool(device.type == "cuda" and not getattr(args, "disable_amp", False))
    scaler = torch.cuda.amp.GradScaler(enabled=amp_enabled)

    epoch_iter = range(1, args.epochs + 1)
    if tqdm is not None:
        epoch_iter = tqdm(epoch_iter, desc="CNN-GRU 训练轮次", dynamic_ncols=True)

    for epoch in epoch_iter:
        model.train()
        train_loss = 0.0
        train_batches: Any = train_loader
        if tqdm is not None:
            train_batches = tqdm(
                train_loader,
        ...
        for X_b, y_b in train_batches:
            ...
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                if loss_mode == "weighted_mse":
                    weighted_alpha = float(getattr(args, "weighted_mse_alpha", 0.15))
                    weighted_cap = float(getattr(args, "weighted_mse_cap", 6.0))
                    weights = 1.0 + weighted_alpha * torch.clamp(torch.abs(y_vec), max=weighted_cap)
                    loss = torch.mean(weights * torch.square(out - y_vec))
                else:
                    loss = criterion(out, y_vec)
        ...
        if valid_loader is not None:
            ...
            valid_loss = 0.0
            with torch.no_grad():
                valid_batches: Any = valid_loader
                if tqdm is not None:
                    valid_batches = tqdm(
                        valid_loader,
                ...
                for X_b, y_b in valid_batches:
                    ...
                    with torch.cuda.amp.autocast(enabled=amp_enabled):
                        ...
                        if loss_mode == "weighted_mse":
                            ...
                            weighted_cap = float(getattr(args, "weighted_mse_cap", 6.0))
                            weights = 1.0 + weighted_alpha * torch.clamp(torch.abs(y_vec), max=weighted_cap)
                            batch_loss = torch.mean(weights * torch.square(out - y_vec))
                        else:
                            batch_loss = criterion(out, y_vec)
                    valid_loss += batch_loss.item() * X_b.size(0)
            ...
            if diag_interval > 0 and epoch % diag_interval == 0:
                ...
                ref_X = X_valid
                if len(ref_X) > 0:
                    sample = ref_X[: min(n_diag, len(ref_X))]
                    with torch.no_grad():
                        sample_pred = model(torch.from_numpy(sample).to(device)).cpu().numpy().astype(np.float32)
                    print(
                        "[DIAG] "
                        f"epoch={epoch} {describe_vector('pred_scaled', sample_pred)} "
        ...
        if device.type == "cuda" and getattr(args, "log_gpu_memory", False):
            log_every = max(1, int(getattr(args, "gpu_log_interval", 1)))
            if epoch % log_every == 0:
                ...
                print(
                    ...
                    f"peak_allocated={max_allocated_gb:.3f}GB"
                )
    ...
    if best_state is not None:
        model.load_state_dict(best_state)
    if save_checkpoint:
        torch.save(model.state_dict(), output_dir / "cnn_gru_best.pt")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using CNN+GRU (GPU/CPU)."
    )
    ...
    parser.add_argument("--diagnostic-interval", type=int, default=10, help="Epoch interval to print prediction diagnostics; <=0 disables.")
    parser.add_argument("--diagnostic-samples", type=int, default=512, help="Max samples used for per-epoch diagnostic prediction stats.")
    ...
    parser.add_argument("--cnn-channels", type=int, nargs="+", default=[64, 128], help="CNN channel sizes per layer.")
    parser.add_argument("--cnn-kernel-sizes", type=int, nargs="+", default=None, help="CNN kernel sizes (default: 3 for each).")
    parser.add_argument("--cnn-pool-size", type=int, default=2, help="MaxPool1d kernel size.")
    parser.add_argument("--gru-hidden", type=int, default=128, help="GRU hidden size.")
    parser.add_argument("--gru-layers", type=int, default=2, help="GRU num layers.")
    ...
    parser.add_argument(
        ...
        type=str,
        default="smoothl1",
        choices=("mse", "smoothl1", "weighted_mse"),
        help="Training loss function.",
    )
    parser.add_argument("--smoothl1-beta", type=float, default=1.0, help="Beta parameter for SmoothL1Loss.")
    parser.add_argument("--weighted-mse-alpha", type=float, default=0.15, help="Weight factor for weighted MSE.")
    parser.add_argument("--weighted-mse-cap", type=float, default=6.0, help="Absolute target cap used in weighted MSE.")
    parser.add_argument(
        "--target-transform",
        type=str,
        default="log1p",
        choices=("none", "log1p"),
        help="Target transform applied before standardization.",
    )
    ...
    parser.add_argument("--log-gpu-memory", action="store_true", help="Log CUDA memory usage every N epochs.")
    parser.add_argument("--gpu-log-interval", type=int, default=1, help="Epoch interval for CUDA memory logging.")
    ...
    parser.add_argument("--tune", action="store_true", help="Run Optuna hyperparameter tuning (single cluster).")
    parser.add_argument("--tune-trials", type=int, default=30, help="Number of Optuna trials when --tune.")
    parser.add_argument("--tune-timeout", type=float, default=None, help="Timeout in seconds for Optuna (optional).")
    parser.add_argument("--tune-cluster", type=str, default=None, help="Cluster name for tuning (default: first available).")
    parser.add_argument("--tune-save-params", type=str, default=None, help="JSON path to save best params (default: output_dir/best_params.json).")
    parser.add_argument("--tune-epochs", type=int, default=None, help="Max epochs per trial when --tune (default: same as --epochs).")
    parser.add_argument("--params-json", type=str, default=None, help="Load hyperparameters from JSON (e.g. from --tune-save-params) for full training.")
    return parser
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="CNN-GRU 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    if getattr(args, "tune", False):
        try:
            ...
            raise ImportError("Optuna is required for --tune. Install with: pip install optuna") from e
        tune_cluster = getattr(args, "tune_cluster", None) or (clusters_to_train[0] if clusters_to_train else None)
        if tune_cluster and tune_cluster not in clusters_to_train:
            tune_cluster = clusters_to_train[0] if clusters_to_train else None
        elif not tune_cluster:
            tune_cluster = clusters_to_train[0] if clusters_to_train else None
        if not tune_cluster:
            raise ValueError("No cluster available for tuning.")
        cluster_train_df = train_df.loc[train_df["cluster"] == tune_cluster].copy()
        cluster_valid_df = valid_df.loc[valid_df["cluster"] == tune_cluster].copy()
        cluster_test_df = test_df.loc[test_df["cluster"] == tune_cluster].copy()
        (
            X_train_tune, y_train_tune, meta_train_tune,
            X_valid_tune, y_valid_tune, meta_valid_tune,
            X_test_tune, y_test_tune, meta_test_tune,
            feature_cols_tune,
            city_categories_tune,
        ) = build_sequence_matrices(cluster_train_df, cluster_valid_df, cluster_test_df, seq_len=args.seq_len)
        if len(X_train_tune) == 0 or len(X_valid_tune) == 0:
            raise ValueError(f"Cluster {tune_cluster} has insufficient train/valid samples for tuning.")
        target_transform_mode = str(getattr(args, "target_transform", "none")).strip().lower()
        y_train_model_t = apply_target_transform(y_train_tune, target_transform_mode)
        y_valid_model_t = apply_target_transform(y_valid_tune, target_transform_mode)
        x_mean_t, x_std_t, y_mean_t, y_std_t = fit_train_standardizers(
            X_train=X_train_tune,
            y_train=y_train_model_t,
            min_std=float(args.min_std),
        )
        X_train_scaled_t = transform_inputs(X_train_tune, x_mean_t, x_std_t)
        X_valid_scaled_t = transform_inputs(X_valid_tune, x_mean_t, x_std_t)
        y_train_scaled_t = transform_targets(y_train_model_t, y_mean_t, y_std_t)
        y_valid_scaled_t = transform_targets(y_valid_model_t, y_mean_t, y_std_t)
        tune_output_dir = output_dir / tune_cluster.lower()
        tune_output_dir.mkdir(parents=True, exist_ok=True)
        max_epochs = getattr(args, "tune_epochs", None) or args.epochs

        def _objective(trial: "optuna.Trial") -> float:
            trial_args = argparse.Namespace(**{**vars(args), "epochs": max_epochs})
            trial_args.lr = trial.suggest_float("lr", 1e-5, 1e-1, log=True)
            trial_args.batch_size = trial.suggest_categorical("batch_size", [128, 256, 512, 1024])
            trial_args.cnn_channels = trial.suggest_categorical(
                "cnn_channels",
                [(32, 64), (64, 128), (64, 128, 128), (128, 256)],
            )
            trial_args.cnn_pool_size = trial.suggest_categorical("cnn_pool_size", [2, 3])
            trial_args.gru_hidden = trial.suggest_categorical("gru_hidden", [64, 128, 256])
            trial_args.gru_layers = trial.suggest_int("gru_layers", 1, 3)
            trial_args.dropout = trial.suggest_float("dropout", 0.1, 0.5)
            trial_args.weight_decay = trial.suggest_float("weight_decay", 1e-5, 1e-2, log=True)
            trial_args.scheduler = trial.suggest_categorical("scheduler", ["plateau", "cosine"])
            trial_args.early_stopping_patience = trial.suggest_int("early_stopping_patience", 5, 15)
            trial_args.diagnostic_interval = 0

            def report_callback(epoch: int, valid_rmse: float) -> None:
                trial.report(valid_rmse, epoch)
                if trial.should_prune():
                    raise optuna.TrialPruned()

            _, best_rmse = train_cnn_gru(
                X_train_scaled_t,
                y_train_scaled_t,
                X_valid_scaled_t,
                y_valid_scaled_t,
        ...
        sampler = optuna.samplers.TPESampler(n_startup_trials=5, n_ei_candidates=24, seed=args.seed)
        ...
        study = optuna.create_study(
            ...
            sampler=sampler,
            pruner=pruner,
            study_name="cnn_gru_daily_pm25",
        )
        study.optimize(
            _objective,
            n_trials=args.tune_trials,
            timeout=getattr(args, "tune_timeout"),
            show_progress_bar=True,
        )
        save_path = getattr(args, "tune_save_params", None) or str(output_dir / "best_params.json")
        Path(save_path).parent.mkdir(parents=True, exist_ok=True)
        with open(save_path, "w", encoding="utf-8") as f:
            json.dump({"best_value": study.best_value, **study.best_params}, f, indent=2)
        print(f"[TUNE] Best valid RMSE: {study.best_value:.4f}, params saved to {save_path}")
        return 0
    ...
    for cluster_name in clusters_to_train:
        ...
        if int(getattr(args, "debug_overfit_n", 0)) > 0:
            ...
            if debug_n > 1:
                ...
                debug_pred_scaled = predict_numpy_seq(debug_model, X_train_scaled[:debug_n], device)
                debug_pred_model = inverse_transform_targets(debug_pred_scaled, y_mean, y_std)
                debug_pred = inverse_target_transform(debug_pred_model, target_transform_mode)
        ...
        pred_train_model = inverse_transform_targets(pred_train_scaled, y_mean, y_std)
        pred_valid_model = inverse_transform_targets(pred_valid_scaled, y_mean, y_std)
        pred_test_model = inverse_transform_targets(pred_test_scaled, y_mean, y_std)
        pred_train = inverse_target_transform(pred_train_model, target_transform_mode)
        pred_valid = inverse_target_transform(pred_valid_model, target_transform_mode)
        pred_test = inverse_target_transform(pred_test_model, target_transform_mode)
        metric_rows = [
    ...
    for cluster_name in target_cluster_order:
        ...
        export_regression_artifacts(all_pred_df=all_pred_df, output_dir=cluster_output_dir, model_name=f"CNNGRU-{cluster_name}")
    ...
    export_regression_artifacts(all_pred_df=all_pred_df, output_dir=output_dir, model_name="CNNGRU-ClusterModels")
    if stage_pbar is not None:
        stage_pbar.update(1)
    ...
    run_info = {
        ...
        "cnn_channels": getattr(args, "cnn_channels", [64, 128]),
        "cnn_kernel_sizes": getattr(args, "cnn_kernel_sizes"),
        "cnn_pool_size": getattr(args, "cnn_pool_size", 2),
        ...
        "weighted_mse_alpha": args.weighted_mse_alpha,
        "weighted_mse_cap": args.weighted_mse_cap,
        "target_transform": args.target_transform,
        "amp_enabled": bool(device.type == "cuda" and not args.disable_amp),
        "early_stopping_patience": args.early_stopping_patience,
        "log_gpu_memory": bool(args.log_gpu_memory),
        "gpu_log_interval": int(args.gpu_log_interval),
    ...
    print("=" * 90)
    print("[INFO] CNN-GRU daily PM2.5 training finished.")
