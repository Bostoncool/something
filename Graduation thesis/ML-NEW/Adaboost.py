"""
Daily PM2.5 prediction for BTH/YRD/PRD using AdaBoost (sklearn) with per-cluster training.
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "adaboost_daily_pm25"
...
def train_adaboost(
    ...
    if getattr(args, "enable_bayes_search", False):
        ...
        search_spaces = {
            ...
            "estimator__max_depth": Integer(1, 8),
        ...
        base_est = AdaBoostRegressor(
            estimator=DecisionTreeRegressor(max_depth=4, random_state=args.seed),
            n_estimators=100,
            learning_rate=0.1,
            loss="linear",
            random_state=args.seed,
    ...
    model = AdaBoostRegressor(**train_params)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using AdaBoost."
    )
    parser.add_argument(
        "--daily-input",
        nargs="+",
    ...
    parser.add_argument("--n-estimators", type=int, default=200, help="Number of boosting estimators.")
    parser.add_argument("--learning-rate", type=float, default=0.1, help="Learning rate.")
    parser.add_argument("--max-depth", type=int, default=4, help="Max depth of base DecisionTreeRegressor.")
    parser.add_argument(
        "--loss",
        type=str,
        default="linear",
        choices=("linear", "square", "exponential"),
        help="Loss function for weight updates.",
    )
    parser.add_argument(
        "--early-stopping-rounds",
        type=int,
        default=0,
        help="Ignored (AdaBoost has no early stopping); kept for API compatibility.",
    )
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="AdaBoost 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    for cluster_name in clusters_to_train:
        ...
        try:
            model, grid_search_info = train_adaboost(x_train, y_train, x_valid, y_valid, args)
    ...
    for cluster_name in target_cluster_order:
        ...
        joblib.dump(model, cluster_output_dir / "adaboost_best.joblib")
        ...
        export_regression_artifacts(
            ...
            model_name=f"AdaBoost-{cluster_name}",
        )
    ...
    export_regression_artifacts(
        ...
        model_name="AdaBoost-ClusterModels",
    )
    if stage_pbar is not None:
        stage_pbar.update(1)
    ...
    print("=" * 90)
    print("[INFO] AdaBoost daily PM2.5 training finished.")
    ...
    print("       - run_info.json")
    print("       - adaboost_best.joblib (per cluster)")
    print("=" * 90)
    return 0
