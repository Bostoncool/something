"""
Daily PM2.5 prediction for BTH/YRD/PRD using pure Temporal Transformer (no city/spatial).
Uses same data pipeline as ST-Transformer: pollution + meteorological only; input (seq_len, n_features).
"""
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "transformer_daily_pm25"
...
class TransformerRegressor(nn.Module):
    """Pure temporal Transformer for sequence regression. Input (B, T, F) -> scalar."""
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if valid_loader is not None:
            ...
            if diag_interval > 0 and epoch % diag_interval == 0:
                ...
                if len(sample_X) > 0:
                    with torch.no_grad():
                        pred_diag = model(torch.from_numpy(sample_X).to(device)).cpu().numpy().astype(np.float32)
                    print(
                        "[DIAG] "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using pure Temporal Transformer (pollution + met only)."
    )
    ...
    parser.add_argument("--debug-overfit-n", type=int, default=0, help="If >0, run overfit check on first N train samples.")
    parser.add_argument("--debug-overfit-epochs", type=int, default=200, help="Epochs for overfit sanity check.")
    parser.add_argument("--debug-overfit-batch-size", type=int, default=64, help="Batch size for overfit check.")
    ...
    parser.add_argument("--tune-cluster", type=str, default=None, help="Cluster for tuning (default: first available).")
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="Transformer 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    if getattr(args, "tune", False):
        ...
        def _objective(trial: "optuna.Trial") -> float:
            ...
            d_model_options = [x for x in [32, 64, 128] if x % nhead == 0]
            d_model = trial.suggest_categorical("d_model", d_model_options if d_model_options else [nhead * 8])
            ...
            trial_args.nhead = nhead
            trial_args.num_encoder_layers = trial.suggest_int("num_encoder_layers", 1, 4)
            trial_args.dim_feedforward = trial.suggest_categorical("dim_feedforward", [128, 256, 512])
        ...
        study = optuna.create_study(
            ...
            sampler=sampler,
            pruner=pruner,
            study_name="transformer_daily_pm25",
        )
        study.optimize(
            _objective,
    ...
    for cluster_name in clusters_to_train:
        ...
        if int(getattr(args, "debug_overfit_n", 0)) > 0:
            ...
            if debug_n > 1:
                ...
                debug_model, _ = train_transformer(
                ...
                debug_pred_scaled = predict_numpy(debug_model, X_train_scaled[:debug_n], device)
        ...
        pred_train_scaled = predict_numpy(model, X_train_scaled, device)
        pred_valid_scaled = predict_numpy(model, X_valid_scaled, device)
        pred_test_scaled = predict_numpy(model, X_test_scaled, device)
    ...
    for cluster_name in target_cluster_order:
        ...
        export_regression_artifacts(all_pred_df=all_pred_df, output_dir=cluster_output_dir, model_name=f"Transformer-{cluster_name}")
    ...
    export_regression_artifacts(all_pred_df=all_pred_df, output_dir=output_dir, model_name="Transformer-ClusterModels")
    if stage_pbar is not None:
        stage_pbar.update(1)
    ...
    print("=" * 90)
    print("[INFO] Transformer daily PM2.5 training finished.")
