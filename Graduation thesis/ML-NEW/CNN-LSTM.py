...
class CNNLSTMRegressor(nn.Module):
    ...
    def __init__(
        ...
        self.feature_pooling = str(feature_pooling).strip().lower() or "attention"
        if self.feature_pooling not in {"mean", "attention"}:
            raise ValueError("feature_pooling must be one of: mean, attention")
        self.feature_pool_gate = nn.Linear(input_size, input_size)
    ...
    def forward(self, x: "torch.Tensor") -> "torch.Tensor":
        ...
        if self.feature_pooling == "mean":
            ...
            pooled_context = x.mean(dim=1)  # (B, T, F)
            feature_weights = torch.softmax(self.feature_pool_gate(pooled_context), dim=-1)  # (B, T, F)
            x = (x * feature_weights.unsqueeze(1)).sum(dim=3)
        # (B, C, T) -> (B, T, C)
        x = x.permute(0, 2, 1)
        out, _ = self.lstm(x)
...
def train_cnn_lstm(
    ...
    model = CNNLSTMRegressor(
        ...
        feature_pooling=getattr(args, "feature_pooling", "attention"),
        seed=args.seed,
    ).to(device)
    ...
    if tqdm is not None:
        epoch_iter = tqdm(epoch_iter, desc="CNN-LSTM 训练轮次", dynamic_ncols=True)
    ...
    for epoch in epoch_iter:
        ...
        if diag_interval > 0 and epoch % diag_interval == 0:
            ...
            ref_X = X_valid if len(X_valid) > 0 else X_train
            if len(ref_X) > 0:
...
def compute_feature_importance_last_step(
    ...
    grad = None
    try:
        # cuDNN RNN/LSTM backward in eval mode is unsupported.
        # Disable cuDNN temporarily for gradient-based importance.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Feature-axis pooling after CNN.",
    )
    parser.add_argument(
        "--loss",
        type=str,
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="CNN-LSTM 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    if getattr(args, "tune", False):
        ...
        def _objective(trial: "optuna.Trial") -> float:
            ...
            trial_args.cnn_channels = trial.suggest_categorical("cnn_channels", [32, 64, 128])
            trial_args.cnn_kernel_time = trial.suggest_int("cnn_kernel_time", 3, 5)
            trial_args.cnn_kernel_feature = trial.suggest_int("cnn_kernel_feature", 3, 5)
            trial_args.lstm_hidden = trial.suggest_categorical("lstm_hidden", [64, 128, 256])
            trial_args.lstm_layers = trial.suggest_int("lstm_layers", 1, 3)
            ...
            trial_args.feature_pooling = trial.suggest_categorical("feature_pooling", ["mean", "attention"])
        ...
        study = optuna.create_study(
            ...
            sampler=sampler,
            pruner=pruner,
            study_name="cnn_lstm_daily_pm25",
        )
        study.optimize(
            _objective,
    ...
    for cluster_name in target_cluster_order:
        ...
        export_regression_artifacts(all_pred_df=all_pred_df, output_dir=cluster_output_dir, model_name=f"CNN_LSTM-{cluster_name}")
    ...
    export_regression_artifacts(all_pred_df=all_pred_df, output_dir=output_dir, model_name="CNN_LSTM-ClusterModels")
    if stage_pbar is not None:
        stage_pbar.update(1)
