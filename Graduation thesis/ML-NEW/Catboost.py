"""
Daily PM2.5 prediction for BTH/YRD/PRD using CatBoost with GPU/CPU.
Reuses daily_ml_pipeline for data preparation and evaluation; per-cluster training.
"""
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "catboost_daily_pm25"
...
def train_catboost(
    ...
    if use_gpu:
        ...
        base_params["thread_count"] = args.n_jobs if args.n_jobs > 0 else -1
    ...
    if getattr(args, "enable_bayes_search", False):
        ...
        search_spaces = {
            ...
            "l2_leaf_reg": Real(0.1, 10.0, prior="log-uniform"),
        }
    ...
    model = CatBoostRegressor(**base_params, **train_params)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using CatBoost."
    )
    parser.add_argument(
        "--daily-input",
        nargs="+",
    ...
    parser.add_argument("--n-estimators", type=int, default=1500, help="Number of boosting iterations.")
    ...
    parser.add_argument("--colsample-bytree", type=float, default=0.8, help="Column sampling ratio per level.")
    parser.add_argument("--reg-lambda", type=float, default=2.0, help="L2 regularization (l2_leaf_reg).")
    parser.add_argument("--early-stopping-rounds", type=int, default=50, help="Early stopping rounds.")
    parser.add_argument("--n-jobs", type=int, default=-1, help="Parallel jobs for model fitting (CPU only).")
    parser.add_argument(
        "--use-gpu",
        action="store_true",
        help="Use GPU for CatBoost (task_type=GPU); 贝叶斯搜索时每折训练也会用 GPU 加速.",
    )
    parser.add_argument(
        "--enable-bayes-search",
        ...
        action="store_true",
        default=False,
        help="Enable Bayesian hyperparameter search (GPU 可加速每折训练).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="贝叶斯搜索 CV 并行数，建议 1 以便 GPU 稳定.",
    )
    parser.add_argument(
        "--bayes-scoring",
        type=str,
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="CatBoost 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    for cluster_name in clusters_to_train:
        ...
        try:
            model, grid_search_info = train_catboost(x_train, y_train, x_valid, y_valid, args)
    ...
    for cluster_name in target_cluster_order:
        ...
        model.save_model(str(cluster_output_dir / "catboost_best.cbm"))
        ...
        export_regression_artifacts(
            ...
            model_name=f"CatBoost-{cluster_name}",
        )
    ...
    export_regression_artifacts(
        ...
        model_name="CatBoost-ClusterModels",
    )
    if stage_pbar is not None:
        stage_pbar.update(1)
    ...
    print("=" * 90)
    print("[INFO] CatBoost daily PM2.5 training finished.")
    ...
    print("       - run_info.json")
    print("       - catboost_best.cbm (per cluster)")
    print("=" * 90)
    return 0
