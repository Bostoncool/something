"""
Daily PM2.5 prediction for BTH/YRD/PRD using TCN (Temporal Convolutional Network).
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "tcn_daily_pm25"
...
class TCNRegressor(nn.Module):
    ...
    def __init__(
        ...
        self._n_cities = max(0, n_cities)
        self.use_city_embed = use_city_embed and self._n_cities > 0
        self.num_channels = num_channels

        # (B, T, F) -> (B, F, T)，再映射到 num_channels
        self.input_proj = nn.Conv1d(input_size, num_channels, 1)
        layers: list[TemporalBlock] = []
        for i in range(num_levels):
            dilation = 2 ** i
            in_ch = num_channels if i == 0 else num_channels
            layers.append(
                TemporalBlock(
                    in_channels=in_ch,
        ...
        if self.use_city_embed:
            self.city_embed = nn.Embedding(max(1, self._n_cities), city_embed_dim)
            self.fc = nn.Linear(num_channels + city_embed_dim, 1)
            self._city_embed_dim = city_embed_dim
        else:
            self.city_embed = None
            self.fc = nn.Linear(num_channels, 1)
            self._city_embed_dim = 0

    def forward(self, x: "torch.Tensor", city_id: "torch.Tensor | None" = None) -> "torch.Tensor":
        # x: (B, T, F)
        # (B, T, F) -> (B, F, T)
        x = x.transpose(1, 2)
        x = self.input_proj(x)
        ...
        h = x[:, :, -1]
        h = self.dropout(h)
        if self.use_city_embed and city_id is not None:
            c_emb = self.city_embed(city_id.clamp(0, self.city_embed.num_embeddings - 1))
            h = torch.cat([h, c_emb], dim=-1)
        return self.fc(h).squeeze(-1)


def build_tcn_dataloaders(
    X_train: np.ndarray,
    y_train: np.ndarray,
...
def train_tcn(
    ...
    n_cities = max(0, len(city_categories))
    model = TCNRegressor(
        ...
        num_levels=getattr(args, "num_levels", 4),
    ...
    train_loader, valid_loader = build_tcn_dataloaders(
    ...
    if best_state is not None:
        model.load_state_dict(best_state)
    if save_checkpoint:
        torch.save(model.state_dict(), output_dir / "tcn_best.pt")
    return model, best_rmse


def predict_numpy_tcn(
    model: "nn.Module",
    X: np.ndarray,
    city_id: np.ndarray,
    device: "torch.device",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using TCN (Temporal Convolutional Network)."
    )
    ...
    parser.add_argument("--debug-overfit-n", type=int, default=0, help="If >0, run overfit sanity check on first N train samples.")
    ...
    # TCN 专用
    parser.add_argument("--num-channels", type=int, default=64, help="TCN channel number per level.")
    parser.add_argument("--kernel-size", type=int, default=3, help="TCN causal convolution kernel size.")
    parser.add_argument("--num-levels", type=int, default=4, help="TCN number of temporal blocks (dilation 2^0..2^(num_levels-1)).")
    ...
    parser.add_argument("--use-city-embed", action="store_true", default=True, help="Use city embedding (default).")
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="TCN 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    for cluster_name in clusters_to_train:
        ...
        if int(getattr(args, "debug_overfit_n", 0)) > 0:
            ...
            if debug_n > 1:
                ...
                debug_pred_scaled = predict_numpy_tcn(
                    debug_model,
        ...
        pred_train_scaled = predict_numpy_tcn(model, X_train_scaled, city_id_train, device)
        pred_valid_scaled = predict_numpy_tcn(model, X_valid_scaled, city_id_valid, device)
        pred_test_scaled = predict_numpy_tcn(model, X_test_scaled, city_id_test, device)
    ...
    for cluster_name in target_cluster_order:
        ...
        export_regression_artifacts(all_pred_df=all_pred_df, output_dir=cluster_output_dir, model_name=f"TCN-{cluster_name}")
    ...
    export_regression_artifacts(all_pred_df=all_pred_df, output_dir=output_dir, model_name="TCN-ClusterModels")
    if stage_pbar is not None:
        stage_pbar.update(1)
    ...
    run_info = {
        ...
        "use_city_embed": args.use_city_embed,
    ...
    print("=" * 90)
    print("[INFO] TCN daily PM2.5 training finished.")
