"""
Daily PM2.5 prediction for BTH/YRD/PRD using MLP (flattened sequence + city embedding).
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "mlp_daily_pm25"
...
class MLPRegressor(nn.Module):
    """MLP for sequence regression: flatten (B, T, F) -> (B, T*F), optional city embedding, then FC layers."""
    ...
    def __init__(
        ...
        if use_city_embed:
            ...
            input_dim = flat_dim + city_embed_dim
        else:
    ...
    def forward(self, x: "torch.Tensor", city_id: "torch.Tensor") -> "torch.Tensor":
        ...
        if self.use_city_embed and self.city_embed is not None:
            ...
            flat = torch.cat([flat, c_emb], dim=1)
...
def build_mlp_dataloaders(
    ...
    X_tr = torch.from_numpy(np.ascontiguousarray(X_train))
    y_tr = torch.from_numpy(np.ascontiguousarray(y_train)).unsqueeze(1)
    c_tr = torch.from_numpy(np.asarray(city_id_train, dtype=np.int64, copy=True)).long()
    ...
    valid_loader = None
    if len(X_valid) > 0:
        X_va = torch.from_numpy(np.ascontiguousarray(X_valid))
        y_va = torch.from_numpy(np.ascontiguousarray(y_valid)).unsqueeze(1)
        c_va = torch.from_numpy(np.asarray(city_id_valid, dtype=np.int64, copy=True)).long()
...
def train_mlp_regressor(
    ...
    train_loader, valid_loader = build_mlp_dataloaders(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using MLP (flattened sequence + city embedding)."
    )
    ...
    parser.add_argument("--hidden-dims", type=str, default="256,128,64", help="MLP hidden layer dimensions, comma-separated.")
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="MLP 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    for cluster_name in clusters_to_train:
        ...
        if int(getattr(args, "debug_overfit_n", 0)) > 0:
            ...
            if debug_n > 1:
                ...
                debug_model, _ = train_mlp_regressor(
                ...
                debug_pred_scaled = predict_numpy_mlp(
                    debug_model,
        ...
        pred_train_scaled = predict_numpy_mlp(model, X_train_scaled, city_id_train, device)
        pred_valid_scaled = predict_numpy_mlp(model, X_valid_scaled, city_id_valid, device)
        pred_test_scaled = predict_numpy_mlp(model, X_test_scaled, city_id_test, device)
    ...
    for cluster_name in target_cluster_order:
        ...
        export_regression_artifacts(all_pred_df=all_pred_df, output_dir=cluster_output_dir, model_name=f"MLP-{cluster_name}")
    ...
    export_regression_artifacts(all_pred_df=all_pred_df, output_dir=output_dir, model_name="MLP-ClusterModels")
    if stage_pbar is not None:
        stage_pbar.update(1)
    ...
    hidden_dims = getattr(args, "hidden_dims", "256,128,64")
    if isinstance(hidden_dims, str):
        hidden_dims_list = [int(x) for x in hidden_dims.split(",") if x.strip()]
    else:
    ...
    print("=" * 90)
    print("[INFO] MLP daily PM2.5 training finished.")
