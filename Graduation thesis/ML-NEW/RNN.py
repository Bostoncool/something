"""
Daily PM2.5 prediction for BTH/YRD/PRD using a GRU/RNN sequence model with GPU/CPU.
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "rnn_daily_pm25"
...
class RNNRegressor(nn.Module):
    """Sequence regression with GRU/RNN: input (B, T, F) -> RNN/GRU -> last hidden -> Dropout -> FC -> 1."""
    ...
    def __init__(
        ...
        if cell_type_norm not in {"gru", "rnn"}:
            raise ValueError(f"Unsupported cell_type: {cell_type}")
        self._cell_type = cell_type_norm
        ...
        self.fc = nn.Linear(rnn_hidden * hidden_mult, 1)
...
def train_rnn(
    ...
    model = RNNRegressor(
        ...
        rnn_hidden=getattr(args, "rnn_hidden", 128),
        rnn_layers=getattr(args, "rnn_layers", 2),
        dropout=getattr(args, "dropout", 0.2),
        bidirectional=getattr(args, "bidirectional", False),
        cell_type=getattr(args, "cell_type", "gru"),
        seed=args.seed,
    ).to(device)
    ...
    if tqdm is not None:
        epoch_iter = tqdm(epoch_iter, desc="RNN 训练轮次", dynamic_ncols=True)
    ...
    if best_state is not None:
        model.load_state_dict(best_state)
    if save_checkpoint:
        torch.save(model.state_dict(), output_dir / "rnn_best.pt")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using RNN/GRU (GPU/CPU)."
    )
    ...
    parser.add_argument("--rnn-hidden", type=int, default=128, help="RNN/GRU hidden size.")
    parser.add_argument("--rnn-layers", type=int, default=2, help="RNN/GRU num layers.")
    parser.add_argument("--dropout", type=float, default=0.3, help="Dropout rate.")
    parser.add_argument("--bidirectional", action="store_true", help="Use bidirectional RNN/GRU.")
    parser.add_argument(
        "--cell-type",
        type=str,
        default="gru",
        choices=("gru", "rnn"),
        help="Recurrent cell type: GRU or vanilla RNN.",
    )
    parser.add_argument(
        "--loss",
        type=str,
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="RNN 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    if getattr(args, "tune", False):
        ...
        def _objective(trial: "optuna.Trial") -> float:
            ...
            trial_args.rnn_hidden = trial.suggest_categorical("rnn_hidden", [64, 128, 256])
            trial_args.rnn_layers = trial.suggest_int("rnn_layers", 1, 3)
    ...
    for cluster_name in target_cluster_order:
        ...
        export_regression_artifacts(all_pred_df=all_pred_df, output_dir=cluster_output_dir, model_name=f"RNN-{cluster_name}")
    ...
    export_regression_artifacts(all_pred_df=all_pred_df, output_dir=output_dir, model_name="RNN-ClusterModels")
    if stage_pbar is not None:
        stage_pbar.update(1)
    ...
    print("=" * 90)
    print("[INFO] RNN daily PM2.5 training finished.")
