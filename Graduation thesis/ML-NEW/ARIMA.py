"""
Daily PM2.5 prediction for BTH/YRD/PRD using ARIMA with per-cluster training.
Time series model: aggregates by (date, cluster), trains ARIMA, broadcasts predictions to (date, city).
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "arima_daily_pm25"
...
def train_and_predict_arima(
    ...
    """Fit ARIMA on train, predict train/valid/test. Returns (fitted_result, pred_train, pred_valid, pred_test)."""
    if SARIMAX is None:
        raise ImportError("ARIMA 需要 statsmodels，请安装: pip install statsmodels") from getattr(
    ...
    # 转为 DataFrame 避免 statsmodels 内部对 ndarray 调用 .values 报错
    # endog 与 exog 必须使用相同索引，否则 statsmodels 报 "indices for endog and exog are not aligned"
    # 使用统一的整数索引确保对齐
    n_train = len(train_agg)
    shared_index = pd.RangeIndex(start=0, stop=n_train, step=1)

    def _to_exog(arr, cols, index=None):
        df = pd.DataFrame(np.asarray(arr, dtype=np.float64), columns=cols)
        df.index = index if index is not None else shared_index
        return df

    endog_train = pd.Series(
        train_agg["pm25"].values.astype(np.float64),
        index=shared_index,
    )
    exog_train = _to_exog(train_exog_scaled, feature_cols, shared_index)
    valid_index = valid_agg["date"].values
    test_index = test_agg["date"].values
    valid_exog_df = _to_exog(valid_exog_scaled, feature_cols, valid_index)
    test_exog_df = _to_exog(test_exog_scaled, feature_cols, test_index)
    ...
    # predicted_mean 可能是 ndarray 或 Series，统一转为 ndarray 避免 .values 报错
    def _to_1d_array(x):
        return np.asarray(x).ravel()

    pred_train = _to_1d_array(result.get_prediction(exog=exog_train).predicted_mean)

    forecast_valid = result.get_forecast(steps=len(valid_agg), exog=valid_exog_df)
    pred_valid = _to_1d_array(forecast_valid.predicted_mean)

    endog_valid = pd.Series(valid_agg["pm25"].values.astype(np.float64), index=valid_index)
    result_valid = result.apply(endog_valid, exog=valid_exog_df)
    forecast_test = result_valid.get_forecast(steps=len(test_agg), exog=test_exog_df)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using ARIMA."
    )
    parser.add_argument(
        "--daily-input",
        nargs="+",
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="ARIMA 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    for cluster_name in clusters_to_train:
        ...
        try:
            ...
            print(f"[WARN] 城市群 {cluster_name} ARIMA 训练失败: {exc}")
            continue
    ...
    for cluster_name in target_cluster_order:
        ...
        joblib.dump(
            ...
            cluster_output_dir / "arima_best.joblib",
        )
        ...
        export_regression_artifacts(
            ...
            model_name=f"ARIMA-{cluster_name}",
        )
    ...
    export_regression_artifacts(
        ...
        model_name="ARIMA-ClusterModels",
    )
    if stage_pbar is not None:
        stage_pbar.update(1)
    ...
    print("=" * 90)
    print("[INFO] ARIMA daily PM2.5 training finished.")
    ...
    print("       - run_info.json")
    print("       - arima_best.joblib (per cluster)")
    print("=" * 90)
    return 0
