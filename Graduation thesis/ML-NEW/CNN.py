"""
Daily PM2.5 prediction for BTH/YRD/PRD using CNN (1D temporal convolution + city embedding).
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "cnn_daily_pm25"
...
class CNNRegressor(nn.Module):
    """1D CNN over time for sequence regression. Input (B, T, F) -> Conv1d over T with F as channels."""
    ...
    def __init__(
        ...
        self.use_city_embed = use_city_embed
        ...
        for out_ch, k in zip(channels, kernel_sizes):
            ...
            in_ch = out_ch
            L = (L + 2 * pad - k) // 1
            L = (L - pool_size) // pool_size + 1
        self.conv = nn.Sequential(*layers)
        self._conv_out_len = L
        self._conv_out_ch = in_ch
        conv_flatten_dim = in_ch * max(1, L)

        self.city_embed = nn.Embedding(max(1, n_cities), city_embed_dim) if use_city_embed else None
        fc_in = conv_flatten_dim + (city_embed_dim if use_city_embed else 0)
    ...
    def forward(self, x: "torch.Tensor", city_id: "torch.Tensor") -> "torch.Tensor":
        ...
        h = self.conv(x)
        h = h.flatten(1)
        if self.use_city_embed and self.city_embed is not None:
            c_emb = self.city_embed(city_id.clamp(0, self._n_cities - 1))
...
def train_cnn(
    ...
    channels = getattr(args, "cnn_channels", (64, 128, 128))
    ...
    model = CNNRegressor(
        ...
        pool_size=int(getattr(args, "cnn_pool_size", 2)),
        dropout=float(getattr(args, "dropout", 0.2)),
        city_embed_dim=int(getattr(args, "city_embed_dim", 16)),
        use_city_embed=bool(getattr(args, "use_city_embed", True)),
        seed=args.seed,
    ).to(device)
    ...
    train_loader, valid_loader = build_cnn_dataloaders(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using CNN (1D temporal convolution + city embedding)."
    )
    ...
    parser.add_argument("--debug-overfit-epochs", type=int, default=200, help="Epochs for overfit sanity check.")
    parser.add_argument("--debug-overfit-batch-size", type=int, default=64, help="Batch size for overfit sanity check.")
    parser.add_argument("--cnn-channels", type=int, nargs="+", default=[64, 128, 128], help="CNN channel sizes per layer.")
    ...
    parser.add_argument("--use-city-embed", action="store_true", default=True, help="Use city embedding (default: True).")
    parser.add_argument("--no-use-city-embed", dest="use_city_embed", action="store_false", help="Disable city embedding.")
    ...
    parser.add_argument("--tune-save-params", type=str, default=None, help="JSON path to save best params.")
    parser.add_argument("--tune-epochs", type=int, default=None, help="Max epochs per trial when --tune.")
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="CNN 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    if getattr(args, "tune", False):
        ...
        (
            ...
            feature_cols_tune, city_categories_tune,
        ...
        x_mean_t, x_std_t, y_mean_t, y_std_t = fit_train_standardizers(
            X_train=X_train_tune, y_train=y_train_tune, min_std=float(args.min_std)
        )
        ...
        y_train_scaled_t = transform_targets(y_train_tune, y_mean_t, y_std_t)
        y_valid_scaled_t = transform_targets(y_valid_tune, y_mean_t, y_std_t)
        city_id_train_t = _city_to_id(meta_train_tune, city_categories_tune)
        city_id_valid_t = _city_to_id(meta_valid_tune, city_categories_tune)
        ...
        def _objective(trial: "optuna.Trial") -> float:
            ...
            trial_args.city_embed_dim = trial.suggest_categorical("city_embed_dim", [8, 16, 32])
    ...
    for cluster_name in clusters_to_train:
        ...
        if int(getattr(args, "debug_overfit_n", 0)) > 0:
            ...
            if debug_n > 1:
                ...
                debug_pred_scaled = predict_numpy_cnn(
                    debug_model,
        ...
        pred_train_scaled = predict_numpy_cnn(model, X_train_scaled, city_id_train, device)
        pred_valid_scaled = predict_numpy_cnn(model, X_valid_scaled, city_id_valid, device)
        pred_test_scaled = predict_numpy_cnn(model, X_test_scaled, city_id_test, device)
    ...
    for cluster_name in target_cluster_order:
        ...
        export_regression_artifacts(all_pred_df=all_pred_df, output_dir=cluster_output_dir, model_name=f"CNN-{cluster_name}")
    ...
    export_regression_artifacts(all_pred_df=all_pred_df, output_dir=output_dir, model_name="CNN-ClusterModels")
    if stage_pbar is not None:
        stage_pbar.update(1)
    ...
    run_info = {
        ...
        "cnn_channels": getattr(args, "cnn_channels", [64, 128, 128]),
        ...
        "use_city_embed": getattr(args, "use_city_embed", True),
    ...
    print("=" * 90)
    print("[INFO] CNN daily PM2.5 training finished.")
