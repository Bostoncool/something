"""
Daily PM2.5 prediction for BTH/YRD/PRD using GRU (Gated Recurrent Unit) sequence model.
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "gru_daily_pm25"
...
class GRURegressor(nn.Module):
    """Pure GRU sequence regressor: (B, T, F) -> GRU -> last hidden -> FC -> 1."""
...
def train_gru(
    ...
    if tqdm is not None:
        epoch_iter = tqdm(epoch_iter, desc="GRU 训练轮次", dynamic_ncols=True)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using GRU (Gated Recurrent Unit)."
    )
    ...
    parser.add_argument("--scheduler", type=str, default="plateau", choices=("plateau", "cosine"), help="LR scheduler type.")
    ...
    parser.add_argument("--smoothl1-beta", type=float, default=1.0, help="Beta for SmoothL1Loss.")
    parser.add_argument("--weighted-mse-alpha", type=float, default=0.15, help="Weight factor for weighted MSE.")
    parser.add_argument("--weighted-mse-cap", type=float, default=6.0, help="Absolute target cap for weighted MSE.")
    parser.add_argument(
        "--target-transform",
        type=str,
        default="log1p",
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="GRU 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    for cluster_name in clusters_to_train:
        ...
        joblib.dump(
            ...
            cluster_output_dir / "gru_scaler.joblib",
        )
    ...
    print("=" * 90)
    print("[INFO] GRU daily PM2.5 training finished.")
