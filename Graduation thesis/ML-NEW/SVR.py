"""
Daily PM2.5 prediction for BTH/YRD/PRD using SVR (Support Vector Regression) with per-cluster training.
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "svr_daily_pm25"
...
def _get_svr_feature_importance(
    model: SVR | LinearSVR,
    x_ref: pd.DataFrame,
    y_ref: pd.Series,
    ...
    """Get feature importance: coef_ for linear kernel, else permutation importance."""
    if hasattr(model, "coef_") and model.coef_ is not None and model.coef_.size > 0:
        return np.abs(model.coef_.flatten()).astype(np.float64)
    ...
    return perm.importances_mean.astype(np.float64)
...
def train_svr(
...
) -> tuple[tuple[Any, StandardScaler, StandardScaler], dict[str, Any]]:
    ...
    use_linear_svr = args.kernel == "linear"
    ...
    if getattr(args, "enable_bayes_search", False):
        ...
        if not use_linear_svr:
            search_spaces["gamma"] = Real(1e-4, 10.0, prior="log-uniform")
        ...
        if use_linear_svr:
            base_est = LinearSVR(C=1.0, epsilon=0.1, max_iter=50000, random_state=args.seed)
        else:
            base_est = SVR(
                kernel=args.kernel,
                C=1.0,
        ...
        search.fit(x_train_scaled, y_train_scaled, callback=progress_cb)
        elapsed = time.perf_counter() - start_time
        ...
        return (model, scaler_x, scaler_y), search_info
    ...
    return (model, scaler_x, scaler_y), {"enabled": False}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using SVR."
    )
    parser.add_argument(
        "--daily-input",
        nargs="+",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Kernel coefficient (scale/auto or float); None uses scale for rbf/poly/sigmoid.",
    )
    parser.add_argument(
        "--early-stopping-rounds",
        type=int,
        default=0,
        help="Ignored (SVR has no early stopping); kept for API compatibility.",
    )
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="SVR 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    for cluster_name in clusters_to_train:
        ...
        _log_bs_msg(f"[INFO] 开始训练城市群 {cluster_name} (train_n={len(x_train)}, valid_n={len(x_valid)})")
        train_begin = time.perf_counter()
        try:
            (model, scaler_x, scaler_y), grid_search_info = train_svr(
        ...
        _log_bs_msg(f"[INFO] {cluster_name} 训练完成 ({train_seconds:.1f}s)")
        ...
        pred_valid = scaler_y.inverse_transform(
            model.predict(x_valid_scaled).reshape(-1, 1)
        ).ravel()
    ...
    for cluster_name in target_cluster_order:
        ...
        joblib.dump(
            ...
            cluster_output_dir / "svr_best.joblib",
        )
        ...
        export_regression_artifacts(
            ...
            model_name=f"SVR-{cluster_name}",
        )

        importance = _get_svr_feature_importance(
            model,
            pd.DataFrame(scaler_x.transform(x_train), columns=feature_cols),
            y_train,
            feature_cols,
            n_repeats=5,
            random_state=args.seed,
    ...
    print("=" * 90)
    print("[INFO] SVR daily PM2.5 training finished.")
    ...
    print("       - run_info.json")
    print("       - svr_best.joblib (per cluster: model + scalers)")
    print("=" * 90)
    return 0
