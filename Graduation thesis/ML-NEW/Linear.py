"""
Daily PM2.5 prediction for BTH/YRD/PRD using Linear Regression with per-cluster training.
...
try:
    ...
    from skopt.space import Categorical  # type: ignore[import-untyped]
except ImportError as _e:
    BayesSearchCV = None  # type: ignore[misc, assignment]
    Categorical = None  # type: ignore[misc, assignment]
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "linear_daily_pm25"
...
def _get_linear_feature_importance(model: LinearRegression, feature_cols: list[str]) -> np.ndarray:
    """Linear regression: use absolute coefficient values as feature importance."""
    coef = model.coef_
    if coef.ndim > 1:
        coef = coef.ravel()
...
def train_linear(
    ...
    if getattr(args, "enable_bayes_search", False):
        if BayesSearchCV is None or Categorical is None:
            raise ImportError(
        ...
        n_iter = getattr(args, "bayes_n_iter", 20)
        ...
        base_est = LinearRegression(fit_intercept=True, positive=False, n_jobs=args.n_jobs)
        ...
        search.fit(x_train_scaled, y_train.values, callback=progress_cb)
        elapsed = time.perf_counter() - start_time
    ...
    model = LinearRegression(**train_params)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Daily PM2.5 prediction for BTH/YRD/PRD using Linear Regression."
    )
    parser.add_argument(
        "--daily-input",
        nargs="+",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Whether to fit intercept (default: True).",
    )
    parser.add_argument(
        "--no-fit-intercept",
        dest="fit_intercept",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Constrains coefficients to be positive.",
    )
    parser.add_argument(
        "--no-positive",
        dest="positive",
        action="store_false",
        help="Allow negative coefficients (default).",
    )
    parser.add_argument(
        "--early-stopping-rounds",
        type=int,
        default=0,
        help="Ignored (Linear has no early stopping); kept for API compatibility.",
    )
    parser.add_argument("--n-jobs", type=int, default=-1, help="Parallel jobs for fitting.")
    parser.add_argument(
        "--enable-bayes-search",
    ...
    parser.add_argument("--bayes-n-iter", type=int, default=20, help="贝叶斯搜索迭代次数.")
...
def main() -> int:
    ...
    stage_pbar = tqdm(total=6, desc="Linear 总进度", dynamic_ncols=True) if tqdm is not None else None
    ...
    for cluster_name in clusters_to_train:
        ...
        try:
            (model, scaler_x), grid_search_info = train_linear(
    ...
    for cluster_name in target_cluster_order:
        ...
        joblib.dump(
            {"model": model, "scaler_x": scaler_x},
            cluster_output_dir / "linear_best.joblib",
        )
        ...
        export_regression_artifacts(
            ...
            model_name=f"Linear-{cluster_name}",
        )
        ...
        importance = _get_linear_feature_importance(model, feature_cols)
    ...
    export_regression_artifacts(
        ...
        model_name="Linear-ClusterModels",
    )
    if stage_pbar is not None:
        stage_pbar.update(1)
    ...
    run_info = {
        ...
        "bayes_n_iter": int(getattr(args, "bayes_n_iter", 20)),
    ...
    print("=" * 90)
    print("[INFO] Linear Regression daily PM2.5 training finished.")
    ...
    print("       - run_info.json")
    print("       - linear_best.joblib (per cluster: model + scaler)")
    print("=" * 90)
    return 0
