...
def build_seasonal_pm25(pm_long: pd.DataFrame) -> pd.DataFrame:
    seasonal = pm_long.copy()
    seasonal["year"] = seasonal["period"].str.extract(r"(\d{4})", expand=False)
    seasonal["month"] = pd.to_numeric(seasonal["period"].str.extract(r"\d{4}(\d{2})", expand=False), errors="coerce")
    seasonal = seasonal.dropna(subset=["year", "month"]).copy()
    seasonal["quarter_num"] = ((seasonal["month"].astype(int) - 1) // 3 + 1).astype(int)
    seasonal["period"] = seasonal["year"] + "Q" + seasonal["quarter_num"].astype(str)
    seasonal = (
        seasonal.groupby(["city_norm", "period"], as_index=False)
        .agg(city=("city", "first"), pm25=("pm25", "mean"))
    )
    seasonal["period"] = seasonal["period"].astype(str)
    return seasonal
...
def read_cluster_geojson(cluster_key: str, geojson_dir: Path) -> gpd.GeoDataFrame:
    ...
    for file_path in files:
        ...
        if gdf.empty:
            continue
        gdf["city"] = gdf["name"] if "name" in gdf.columns else file_path.stem
...
def sort_periods(periods: list[str]) -> list[str]:
    def _key(p: str) -> tuple[int, int]:
        if "Q" in p and len(p) >= 6 and p[:4].isdigit():
            year = int(p[:4])
            q = int(p.split("Q")[1])
            return (year, q)
        return (9999, 9)
...
def run_pipeline(pm25_csv: Path, output_dir: Path, alpha: float) -> None:
    ...
    pm_seasonal = build_seasonal_pm25(pm_long)
    periods_sorted = sort_periods(pm_seasonal["period"].unique().tolist())
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="三大城市群季度PM2.5 全局莫兰 + 局部莫兰 + 时空莫兰计算与可视化"
    )
    parser.add_argument("--pm25-csv", type=Path, default=Path(DEFAULT_PM25_CSV), help="PM2.5数据CSV路径（月度）")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "三大城市群_（季）莫兰结果",
        help="输出目录",
    )
