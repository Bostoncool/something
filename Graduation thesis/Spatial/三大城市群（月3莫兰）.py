...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="三大城市群 PM2.5 全局莫兰 + 局部莫兰 + 时空莫兰(STMI)计算与可视化"
    )
    parser.add_argument("--pm25-csv", type=Path, default=Path(DEFAULT_PM25_CSV), help="月均PM2.5数据CSV路径")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "三大城市群_（月）莫兰结果",
        help="输出目录",
    )
