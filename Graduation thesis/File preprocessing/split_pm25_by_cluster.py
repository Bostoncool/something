...
DEFAULT_INPUT_DIR = DATA_ROOT / "2018-2023[PM2.5-china]" / "Day"
DEFAULT_MAP_ROOT = DATA_ROOT / "地图数据"
DEFAULT_OUTPUT_ROOT = DATA_ROOT / "2018-2023[PM2.5-china-clusters]"
DEFAULT_SUMMARY_FILE = "run_summary.json"
DEFAULT_WRITE_ENGINE = "h5netcdf"

DATE_PATTERN = re.compile(r"CHAP_PM2\.5_D1K_(\d{8})(?:_V\d+)?\.nc$", re.IGNORECASE)
...
def _choose_pm25_var(dataset: xr.Dataset) -> str:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="将全国 PM2.5 日尺度 NC 数据按 BTH/YRD/PRD 三大城市群进行掩膜裁剪并分目录保存。"
    )
...
def main() -> int:
    ...
    with ProcessPoolExecutor(
        ...
        progress = tqdm(total=len(future_map), desc="切割 PM2.5 NC", dynamic_ncols=True) if tqdm is not None else None
