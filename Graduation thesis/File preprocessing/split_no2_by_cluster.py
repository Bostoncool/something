...
DEFAULT_INPUT_DIR = Path(r"G:\基本用不到\NO2\Day")
DEFAULT_MAP_ROOT = Path(r"F:\1.模型要用的\地图数据")
DEFAULT_OUTPUT_ROOT = Path(r"F:\1.模型要用的\[2018-2023]NO2-clusters")
DEFAULT_SUMMARY_FILE = "run_summary.json"
DEFAULT_WRITE_ENGINE = "h5netcdf"

DATE_PATTERN = re.compile(r"CHAP_NO2_D1K_(\d{8})(?:_V\d+)?\.nc$", re.IGNORECASE)
NO2_VAR_CANDIDATES = ("NO2", "no2")
...
def _choose_no2_var(dataset: xr.Dataset) -> str:
    for var_name in NO2_VAR_CANDIDATES:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="将全国 NO2 日尺度 NC 数据按 BTH/YRD/PRD 三大城市群进行掩膜裁剪并分目录保存（保留原始数据）。"
    )
...
def main() -> int:
    ...
    with ProcessPoolExecutor(
        ...
        progress = tqdm(total=len(future_map), desc="切割 NO2 NC", dynamic_ncols=True) if tqdm is not None else None
