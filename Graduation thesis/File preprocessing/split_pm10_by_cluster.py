...
DEFAULT_INPUT_DIR = Path(r"G:\基本用不到\PM10\Day")
DEFAULT_MAP_ROOT = Path(r"F:\1.模型要用的\地图数据")
DEFAULT_OUTPUT_ROOT = Path(r"F:\1.模型要用的\[2018-2023]PM10-clusters")
DEFAULT_SUMMARY_FILE = "run_summary.json"
DEFAULT_WRITE_ENGINE = "h5netcdf"

DATE_PATTERN = re.compile(r"CHAP_PM10_D1K_(\d{8})(?:_V\d+)?\.nc$", re.IGNORECASE)
PM10_VAR_CANDIDATES = ("PM10", "pm10")
...
def _choose_pm10_var(dataset: xr.Dataset) -> str:
    for var_name in PM10_VAR_CANDIDATES:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="将全国 PM10 日尺度 NC 数据按 BTH/YRD/PRD 三大城市群进行掩膜裁剪并分目录保存（保留原始数据）。"
    )
...
def main() -> int:
    ...
    with ProcessPoolExecutor(
        ...
        progress = tqdm(total=len(future_map), desc="切割 PM10 NC", dynamic_ncols=True) if tqdm is not None else None
