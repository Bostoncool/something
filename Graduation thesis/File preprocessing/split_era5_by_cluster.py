...
DATA_ROOT = SCRIPT_DIR.parent / "1.模型要用的"
DEFAULT_INPUT_DIR = DATA_ROOT / "2018-2023[ERA5_PM2.5]"
DEFAULT_MAP_ROOT = DATA_ROOT / "地图数据"
DEFAULT_OUTPUT_ROOT = DATA_ROOT / "2018-2023[ERA5_PM2.5-clusters]"
DEFAULT_SUMMARY_FILE = "run_summary.json"
DEFAULT_WRITE_ENGINE = "h5netcdf"

DATE_PATTERN = re.compile(r"(20\d{2})(\d{2})(\d{2})")
PM25_DATE_PATTERN = re.compile(r"CHAP_PM2\.5_D1K_(\d{8})(?:_V\d+)?\.nc$", re.IGNORECASE)
...
def parse_date_from_filename(file_path: Path) -> datetime | None:
    # ERA5 常见文件名: xxx_YYYYMM.nc, xxx_YYYYMMDD.nc
    name = file_path.stem
    m8 = re.search(r"(20\d{2})(\d{2})(\d{2})", name)
    if m8:
        try:
            return datetime.strptime(m8.group(0), "%Y%m%d")
        except ValueError:
            pass
    m6 = re.search(r"(20\d{2})(\d{2})", name)
    if m6:
        try:
            return datetime.strptime(m6.group(0) + "01", "%Y%m%d")
        except ValueError:
            pass

    match_pm = PM25_DATE_PATTERN.search(file_path.name)
    if match_pm:
        try:
            return datetime.strptime(match_pm.group(1), "%Y%m%d")
        except ValueError:
            return None
    return None
...
def _choose_main_var(dataset: xr.Dataset, var_folder: str) -> str:
    alias_map = {
        ...
        "total_cloud_cover": ("total_cloud_cover", "tcc"),
        "total_precipitation": ("total_precipitation", "tp"),
    }
    for candidate in alias_map.get(var_folder, ()):
...
def _build_output_path(output_root: Path, var_folder: str, cluster: str, date_dt: datetime, file_name: str) -> Path:
    return (
        output_root
        / var_folder
        / cluster
...
def _process_single_file(file_path_str: str) -> FileProcessResult:
    ...
    var_folder = file_path.parent.name.strip()
    if date_dt is None:
        detail = ClusterProcessResult(cluster="ALL", status="failed", output_path="", var_folder=var_folder, error="文件名无法解析日期")
    ...
    if dry_run:
        planned = [
            ClusterProcessResult(
                ...
                status="dry_run",
                var_folder=var_folder,
                output_path=str(_build_output_path(output_root, var_folder, cluster, date_dt, file_path.name)),
            )
    ...
    try:
        with _open_dataset_with_fallback(file_path) as ds:
            var_name = _choose_main_var(ds, var_folder=var_folder)
            source_var = ds[var_name]
            for cluster, geometry in cluster_geometries.items():
                output_path = _build_output_path(output_root, var_folder, cluster, date_dt, file_path.name)
...
def _cluster_size_bytes(output_root: Path, cluster: str) -> int:
    if not output_root.exists():
        return 0
    total = 0
    for var_dir in output_root.iterdir():
        if not var_dir.is_dir():
            continue
        cluster_dir = var_dir / cluster
        if not cluster_dir.exists():
            continue
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="将 ERA5 NC 数据按 BTH/YRD/PRD 三大城市群进行掩膜裁剪，输出保留变量目录层级。"
    )
    parser.add_argument("--input-dir", type=str, default=str(DEFAULT_INPUT_DIR), help=f"输入 ERA5 根目录（默认: {DEFAULT_INPUT_DIR}）")
    ...
    parser.add_argument(
        "--verbose-summary",
        action="store_true",
        help="summary 中写入 file_details 全量明细（默认仅写聚合统计，避免 summary 过大）",
    )
    return parser
...
def main() -> int:
    ...
    with ProcessPoolExecutor(
        ...
        progress = tqdm(total=len(future_map), desc="切割 ERA5 NC", dynamic_ncols=True) if tqdm is not None else None
        for future in as_completed(future_map):
            try:
                result = future.result()
            except Exception as exc:  # pylint: disable=broad-except
                fail_path = future_map[future]
                fail_var = Path(fail_path).parent.name
                result = FileProcessResult(
                    ...
                    failed_count=3,
                    details=[
                        ClusterProcessResult(cluster="BTH", status="failed", output_path="", var_folder=fail_var, error=str(exc)),
                        ClusterProcessResult(cluster="YRD", status="failed", output_path="", var_folder=fail_var, error=str(exc)),
                        ClusterProcessResult(cluster="PRD", status="failed", output_path="", var_folder=fail_var, error=str(exc)),
                    ],
                )
    ...
    by_var_folder: dict[str, dict[str, int]] = {}
    for file_result in all_results:
        stats = by_var_folder.setdefault(file_result.var_folder, {"ok": 0, "skip": 0, "failed": 0})
        stats["ok"] += int(file_result.success_count)
        stats["skip"] += int(file_result.skipped_count)
        stats["failed"] += int(file_result.failed_count)
    ...
    if args.verbose_summary:
        summary["file_details"] = [asdict(item) for item in all_results]
